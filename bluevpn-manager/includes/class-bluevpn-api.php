...
final class BlueVPN_API {
    ...
    public static function headers($response,$server,$request){
        if ($request instanceof WP_REST_Request && (str_starts_with($request->get_route(),'/bluevpn/') || str_starts_with($request->get_route(),'/bluevpn-system/'))) {
            $response->header('Content-Language','fa-IR'); $response->header('X-BlueVPN-Timezone','Asia/Tehran'); $response->header('X-BlueVPN-Calendar','jalali'); $response->header('Cache-Control','no-store');
        }
        return $response;
    }
    ...
    public static function account(WP_REST_Request $r): WP_REST_Response { try{$c=BlueVPN_Auth::current_customer($r);return self::ok(['success'=>true,'account'=>BlueVPN_Auth::account_payload($c)]);}catch(BlueVPN_Auth_Exception $e){return self::fail($e);} }
    ...
    public static function subscription(WP_REST_Request $r): WP_REST_Response { global $wpdb;$t=BlueVPN_DB::table('customers');$token=(string)$r['token'];$c=$wpdb->get_row($wpdb->prepare("SELECT subscription_url FROM {$t} WHERE subscription_token=%s AND active=1 LIMIT 1",$token),ARRAY_A);if(!$c)return self::ok(['detail'=>['code'=>'SUBSCRIPTION_NOT_FOUND','message'=>'اشتراک پیدا نشد']],404);return self::ok(['detail'=>['code'=>'PROVIDERS_NOT_MIGRATED','message'=>'اتصال PasarGuard/Marzban/GuardCore هنوز در مرحله بعدی مهاجرت فعال می‌شود.']],503); }
}
