...
## 4.9.8 WARP failure diagnostics
...
## 4.9.8 — Production Runtime Validation
...
## 4.9.8 — Mahsa-Core Canary
Production remains Stock Xray. Manual CI can build an isolated `mahsa-canary` APK using a pinned GFW-knocker AndroidLibXrayLite/Mahsa-Core source. BlueAI records core-separated success, latency, jitter and loss by network/operator and recommends Promote/Reject only after real canary evidence; full releases remain stock-only.
