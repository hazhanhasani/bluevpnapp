...
final class BlueVPN_Admin {
    ...
    public static function add_plan(): void {
        ...
        $pg=max(0,(int)($_POST['panel_id']??0));$mz=max(0,(int)($_POST['marzban_panel_id']??0));$gc=max(0,(int)($_POST['guardcore_panel_id']??0));$services=$ids('guardcore_service_ids');$groups=$postedIds('group_ids_selected','group_ids');$marzbanInbounds=$mzSelected();$mode=in_array((string)($_POST['multi_provider_quota_mode']??'split'),['split','full'],true)?(string)$_POST['multi_provider_quota_mode']:'split';
