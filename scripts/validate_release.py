...
def main() -> None:
    ...
    require("No free loopback port in" in warp and "PORT_MIN = 1819" in warp, "WARP loopback port ownership/range hardening missing")
    require("--quick-reconnect" in warp and "--no-quick-reconnect" in warp, "Aether non-interactive quick-reconnect policy missing")
    require("socksGreetingAndRemoteConnect" in warp and "warp=plus" in warp, "WARP data-plane validation is incomplete")
    require("warpFallbackUntilElapsed" not in home and "warpFallbackGeneration" in home, "fixed 30s WARP fallback window still exists")
