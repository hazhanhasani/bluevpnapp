...
def main() -> None:
    ...
    require(version == "4.1.0" and code == 40100, "this stabilization source must start at 4.1.0 / 40100")
    require(app.get("upstream_ref") == "2.2.6", "production upstream must remain pinned to reviewed stable v2rayNG 2.2.6")
    require(app.get("xray_ref") == "v26.7.28", "Xray backport must be pinned to v26.7.28")
    require("v2rayng-2.3.3-compatibility-reviewed" in release.get("features", []), "2.3.3 compatibility review marker missing")
    require("xray-core-v26.7.28-backport" in release.get("features", []), "Xray 26.7.28 backport marker missing")

    require("Version: 4.1.0" in manager, "WordPress plugin header version mismatch")
    require("BLUEVPN_MANAGER_VERSION', '4.1.0'" in manager, "WordPress plugin constant mismatch")
    require("BLUEVPN_MANAGER_SCHEMA_VERSION', '1.6.0'" in manager, "WordPress schema must be 1.6.0")
    require("Version: 4.1.0" in manager_readme and "Stable tag: 4.1.0" in manager_readme, "WordPress readme version/stable tag mismatch")
