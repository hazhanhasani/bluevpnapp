...
final class BlueVPN_Admin {
    ...
    public static function menu(): void {
        ...
        add_submenu_page('bluevpn-manager','خطاها و پایش','خطاها و پایش','manage_options','bluevpn-error-monitor',[BlueVPN_Error_Monitor::class,'admin_page']);
    ...
    public static function settings_page(): void {
        ...
        echo '<section class="bluevpn-settings-section"><h2>هویت اپلیکیشن</h2><p>نام و آدرس عمومی مرکز کنترل.</p><table class="form-table">';
        echo '<tr><th>نام اپ</th><td><input class="regular-text" type="text" name="app_name" value="'.esc_attr((string)$s['app_name']).'"></td></tr>';
        echo '<tr><th>نشانی پایه عمومی</th><td><input class="regular-text" dir="ltr" type="url" name="public_base_url" value="'.esc_attr((string)$s['public_base_url']).'"><p class="description">نسخه و فایل‌های نصب در «اپ و انتشار» مدیریت می‌شوند.</p></td></tr>';
        echo '</table></section>';
        ...
        $links=[
            ...
            'اتصال رایگان / وارپ'=>'bluevpn-free-access',
            'تبلیغات'=>'bluevpn-ads',
        ...
        echo '<section class="bluevpn-settings-section"><h2>نشانی‌های فنی سیستم</h2><p>برای عیب‌یابی و اتصال سرویس‌ها.</p>';
    ...
    public static function app_connection_page(): void {
        ...
        echo '</div>';
        echo '<div class="bvp-card" style="max-width:900px;margin-top:18px"><h2>مرکز کنترل فعال</h2><pre class="bvp-endpoint">"api_base_url": "'.esc_html($site).'"</pre><p>این Endpoint تنها مرجع فعال BlueVPN است؛ Migration Bridge بازنشسته شده است.</p></div>';
        self::foot();
    }
    ...
    public static function add_plan(): void {
        ...
        $routes=BlueVPN_Control_Center::posted_provider_routes();
        $routeStorage=BlueVPN_Control_Center::provider_route_storage($routes);
        $sourceRaw=$_POST['source_ids_selected']??[];if(!is_array($sourceRaw))$sourceRaw=[];$sourceIds=[];foreach($sourceRaw as $v){$n=(int)$v;if($n>0&&!in_array($n,$sourceIds,true))$sourceIds[]=$n;}
        ...
        $gatewayReplicas=max(1,min(3,(int)($_POST['gateway_replica_count']??2)));
        $usd=round(max(0,(float)($_POST['usd_price']??0)),6);
        try{$price=BlueVPN_Dollar_Pricing::quote_toman($usd);}catch(Throwable $e){wp_safe_redirect(add_query_arg('cc_error',$e->getMessage(),admin_url('admin.php?page=bluevpn-plans')));exit;}
        $data=$routeStorage+[
            'title'=>sanitize_text_field(wp_unslash($_POST['title']??'')),
            'description'=>sanitize_textarea_field(wp_unslash($_POST['description']??'')),
            'price_toman'=>$price,'usd_price'=>$usd,'usd_managed'=>1,'usd_last_price_toman'=>$price,'usd_updated_at'=>BlueVPN_Utils::now_mysql(),
            'duration_days'=>max(0,(int)($_POST['duration_days']??0)),'data_limit_gb'=>max(0,(int)($_POST['data_limit_gb']??0)),
            'device_limit'=>max(1,min(20,(int)($_POST['device_limit']??1))),'active'=>1,'deleted'=>0,'sort_order'=>(int)($_POST['sort_order']??0),
            'marzban_quota_mode'=>$mode,'multi_provider_quota_mode'=>$mode,'traffic_mode'=>$trafficMode,'gateway_replica_count'=>$gatewayReplicas,
            'source_ids_json'=>BlueVPN_Utils::json_encode($sourceIds),'created_at'=>BlueVPN_Utils::now_mysql(),
        ];
        $ok=$wpdb->insert(BlueVPN_DB::table('plans'),$data);
        wp_safe_redirect(add_query_arg($ok===false?'cc_error':'cc_msg',$ok===false?'افزودن پلن ناموفق بود.':'پلن چندمسیره اضافه شد.',admin_url('admin.php?page=bluevpn-plans')));exit;
    }
    ...
    public static function migration_page(): void {
        ...
        echo '<div class="bvp-grid">';
        echo '<div class="bvp-card"><h3>مرکز کنترل</h3><p class="bvp-ok">WordPress / MySQL</p><small>حالت Native و دائمی</small></div>';
