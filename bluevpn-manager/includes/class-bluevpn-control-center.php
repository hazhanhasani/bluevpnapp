...
final class BlueVPN_Control_Center {
    ...
    private static function tab_plans(): void {
        ...
        self::input('group_ids','PasarGuard Group IDs (Fallback)','');
        self::input('guardcore_service_ids','GuardCore Service IDs','');
        echo '</div><div class="bvc-helper">PasarGuard در حالت عادی همه گروه‌های فعال را خودکار از پنل می‌گیرد؛ Group ID فقط Fallback است. Marzban نیز همه Inboundهای فعال را خودکار تخصیص می‌دهد. اگر GuardCore روی حالت خودکار است، Service ID لازم است.</div></details>';
        ...
        foreach($rows as $x){
            ...
            echo '<label>PasarGuard Group IDs (Fallback)<input name="group_ids" value="'.esc_attr($groups).'" placeholder="فقط در صورت عدم دسترسی API"></label>';
            ...
            echo '<label>PasarGuard Group IDs (Fallback)<input name="group_ids" value="'.esc_attr($groups).'"></label><label>GuardCore Service IDs<input name="guardcore_service_ids" value="'.esc_attr($serviceIds).'"></label>';
