...
final class BlueVPN_Admin {
    ...
    public static function add_plan(): void {
        ...
        $pg=max(0,(int)($_POST['panel_id']??0));$mz=max(0,(int)($_POST['marzban_panel_id']??0));$gc=max(0,(int)($_POST['guardcore_panel_id']??0));$services=$postedIds('guardcore_service_ids_selected','guardcore_service_ids');$groups=$postedIds('group_ids_selected','group_ids');$marzbanInbounds=$mzSelected();$mode=sanitize_key((string)wp_unslash($_POST['multi_provider_quota_mode']??'split'));$mode=in_array($mode,['split','full'],true)?$mode:'split';
