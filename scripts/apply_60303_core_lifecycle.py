...
old_snapshot = """        $oldSourceLines=is_array($old['source_lines']??null)?$old['source_lines']:[];
...
"""
new_snapshot = """        $currentSourceKeys=[];foreach($sources as $source){$sourceKey=(string)($source['key']??'source');if($sourceKey!=='')$currentSourceKeys[$sourceKey]=true;}
...
"""
s = one(s, old_snapshot, new_snapshot, "snapshot lkg pruning")
save(p, s)
