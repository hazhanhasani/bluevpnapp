...
struct HomeView: View {
    ...
    var body: some View { ZStack { BlueBackground(); ScrollView(showsIndicators: false) { VStack(spacing: 18) {
        HStack { Button { store.path.append(.account) } label: { Image(systemName: "person").font(.title2).frame(width: 54,height:54).background(.thinMaterial).clipShape(RoundedRectangle(cornerRadius:18)) }; Spacer(); Text("5.7.9").foregroundStyle(.secondary); Button { store.path.append(.settings) } label: { Image(systemName:"line.3.horizontal").font(.title2).frame(width:54,height:54).background(.thinMaterial).clipShape(RoundedRectangle(cornerRadius:18)) } }
