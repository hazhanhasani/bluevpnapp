...
actor APIClient {
    ...
    func text(from absoluteURL: String, token: String? = nil) async throws -> String {
        ...
        var request=URLRequest(url:url);request.setValue("BlueVPN-iOS/5.7.2",forHTTPHeaderField:"User-Agent");request.setValue("text/plain,*/*;q=0.8",forHTTPHeaderField:"Accept")
    ...
    private func request<Body: Encodable, T: Decodable>(_ path: String, method: String, token: String?, body: Body?, as: T.Type) async throws -> T {
        ...
        request.setValue("BlueVPN-iOS/5.7.2", forHTTPHeaderField: "User-Agent")
