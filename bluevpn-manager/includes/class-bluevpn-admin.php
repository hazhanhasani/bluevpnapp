...
final class BlueVPN_Admin {
    ...
    public static function add_plan(): void {
        ...
        $pg=max(0,(int)($_POST['panel_id']??0));$mz=max(0,(int)($_POST['marzban_panel_id']??0));$gc=max(0,(int)($_POST['guardcore_panel_id']??0));$services=$postedIds('guardcore_service_ids_selected','guardcore_service_ids');$groups=$postedIds('group_ids_selected','group_ids');$marzbanInbounds=$mzSelected();$mode=in_array((string)($_POST['multi_provider_quota_mode']??'split'),['split','full'],true)?(string)$_POST['multi_provider_quota_mode']:'split';
        ...
        $ok=$wpdb->insert(BlueVPN_DB::table('plans'),['title'=>sanitize_text_field(wp_unslash($_POST['title']??'')),'description'=>sanitize_textarea_field(wp_unslash($_POST['description']??'')),'price_toman'=>$price,'duration_days'=>max(0,(int)($_POST['duration_days']??0)),'data_limit_gb'=>max(0,(int)($_POST['data_limit_gb']??0)),'device_limit'=>max(1,min(10,(int)($_POST['device_limit']??1))),'group_ids_json'=>BlueVPN_Utils::json_encode($groups),'marzban_inbounds_json'=>BlueVPN_Utils::json_encode($marzbanInbounds),'active'=>1,'deleted'=>0,'sort_order'=>(int)($_POST['sort_order']??0),'panel_id'=>$pg?:null,'marzban_panel_id'=>$mz?:null,'marzban_quota_mode'=>$mode,'guardcore_panel_id'=>$gc?:null,'guardcore_service_ids_json'=>BlueVPN_Utils::json_encode($services),'multi_provider_quota_mode'=>$mode,'created_at'=>BlueVPN_Utils::now_mysql()]);
