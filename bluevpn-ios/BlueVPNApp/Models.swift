...
struct AccountSnapshot: Codable {
    var phone=""; var email=""; var active=false; var tier:PlanTier = .free; var locations=0; var remainingDays=0; var unlimited=false; var planTitle="";var subscriptionURL="";var poolIdentity=""
    init() {}
    init(_ value:AccountPayload){ phone=value.phoneDisplay ?? value.phone ?? "";email=value.email ?? "";active=value.subscription?.active ?? false;tier=active ? .premium:.free;unlimited=value.subscription?.unlimited ?? false;planTitle=value.planTitle ?? "";subscriptionURL=value.subscription?.url ?? "";poolIdentity=value.subscription?.poolIdentity ?? "";if let seconds=value.subscription?.remainingSeconds{remainingDays=max(0,Int(ceil(Double(seconds)/86_400)))} }
}
...
struct SubscriptionPayload:Decodable{let active,unlimited:Bool;let remainingSeconds:Int?;let url,poolIdentity:String?;enum CodingKeys:String,CodingKey{case active,unlimited,url;case remainingSeconds="remaining_seconds";case poolIdentity="pool_identity"}}
