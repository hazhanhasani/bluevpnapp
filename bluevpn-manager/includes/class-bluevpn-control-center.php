...
final class BlueVPN_Control_Center {
    private const TABS=[
        'overview'=>'نمای کلی','blueai'=>'هوش مصنوعی بلووی‌پی‌ان','ads'=>'تبلیغات','free'=>'اتصال رایگان','database'=>'پایگاه داده','production'=>'سلامت و پشتیبان‌گیری','panels'=>'سرور پاسارگاد','marzban'=>'سرور مرزبان','hiddify'=>'سرور هیدیفای','threexui'=>'سرور 3x-ui','shahrah'=>'تأمین‌کننده شاهراه','guardcore'=>'سرور گاردکور','guardcore-manual'=>'صف گاردکور','sources'=>'منابع اشتراک','gateway'=>'دروازه اندازه‌گیری مصرف','plans'=>'پلن‌ها','blupal'=>'درگاه پرداخت بلوپال','manual'=>'فعال‌سازی دستی','customers'=>'کاربران','manual-customers'=>'مشتریان دستی','orders'=>'سفارش‌ها و پرداخت‌ها','android'=>'اندروید و انتشار','windows'=>'ویندوز و انتشار','app'=>'انتشار برنامه‌ها','sms'=>'پیامک و رمز یک‌بارمصرف'
    ];
    private const PAGE_SLUGS=[
        ...
        'hiddify'=>'bluevpn-hiddify',
        'threexui'=>'bluevpn-threexui',
        'shahrah'=>'bluevpn-shahrah',
        'guardcore'=>'bluevpn-guardcore',
        'guardcore-manual'=>'bluevpn-guardcore-queue',
        'sources'=>'bluevpn-subscription-sources',
        'gateway'=>'bluevpn-gateway',
        'plans'=>'bluevpn-plans',
        ...
        'android'=>'bluevpn-android-update',
        'windows'=>'bluevpn-windows-update',
    ...
    public static function init(): void {
        foreach([
            ...
            'bluevpn_cc_manual_activate'=>'manual_activate','bluevpn_cc_update_customer_entitlement'=>'update_customer_entitlement','bluevpn_cc_retry_order_provision'=>'retry_order_provision','bluevpn_cc_attach_guardcore'=>'attach_guardcore',
    ...
    private static function dt($v,bool $withSeconds=true): string { $f=BlueVPN_Utils::tehran_datetime_fa($v,$withSeconds);return self::esc($f!==''?$f:'—'); }
    ...
    private static function sync_error_text($v): string {
        $s=(string)$v;
        $s=str_replace(['(INVALID_JSON)','INVALID_JSON'],['(پاسخ غیر JSON قدیمی شاهراه)','پاسخ غیر JSON قدیمی شاهراه'],$s);
        return $s;
    }
    ...
    private static function tab_overview(): void {
        global $wpdb;$stats=[
            ['کاربران',self::count('customers')],['مشتریان دستی',self::count('manual_customers')],['اشتراک فعال',self::count('customers',"active=1 AND subscription_status='active'")],['پرداخت موفق',self::count('orders',"status IN ('paid','activated','paid_needs_sync','partial_needs_sync')")],['پلن فعال',self::count('plans','active=1 AND deleted=0')],['Provider فعال',self::count('pasarguard_panels','active=1')+self::count('marzban_panels','active=1')+self::count('hiddify_panels','active=1')+self::count('threexui_panels','active=1')+self::count('shahrah_panels','active=1')+self::count('guardcore_panels','active=1')],['اتصال زنده',self::count('ai_live_connections','connected=1 AND verified=1')]
        ];
        ...
        echo '<div class="bvc-card"><h2>آخرین پرداخت‌ها</h2><table class="widefat striped bvc-table"><tr><th>سفارش</th><th>مبلغ</th><th>وضعیت</th><th>زمان</th></tr>';foreach($orders as $x)echo '<tr><td>'.self::esc($x['order_code']).'</td><td>'.number_format((int)$x['amount_toman']).'</td><td>'.self::esc($x['status']).'</td><td>'.self::dt($x['created_at']).'</td></tr>';echo '</table></div>';
    }
    ...
    private static function tab_production(): void {
        ...
        echo '<div class="bvc-card"><h3>Backup خودکار</h3><p class="'.(!empty($backup['ok'])?'bvc-ok':'bvc-warn').'">'.(!empty($backup['ok'])?'فعال و سالم':'هنوز Backup سالم ثبت نشده').'</p><small>'.self::dt($backup['created_at']??'').'</small></div>';
        echo '<div class="bvc-card"><h3>Restore</h3><p class="'.(!empty($restore['ok'])?'bvc-ok':'bvc-warn').'">'.(!empty($restore['ok'])?'آخرین Restore موفق':'Restore اجرا نشده یا آخرین اجرا ناموفق بوده').'</p><small>'.self::dt($restore['at']??'').'</small></div>';
    ...
    private static function provider_tab(string $provider): void {
        global $wpdb;
        $maps=[
            'pasarguard'=>['pasarguard_panels','PasarGuard'],
            'marzban'=>['marzban_panels','Marzban'],
            'guardcore'=>['guardcore_panels','GuardCore'],
            'hiddify'=>['hiddify_panels','Hiddify Manager'],
            'threexui'=>['threexui_panels','3x-ui'],
        ];
        if(!isset($maps[$provider])){echo '<div class="notice notice-error"><p>Provider نامعتبر است.</p></div>';return;}
        [$table,$title]=$maps[$provider];$t=BlueVPN_DB::table($table);
        ...
        echo '<input type="hidden" name="action" value="bluevpn_cc_save_provider"><input type="hidden" name="provider" value="'.self::esc($provider).'"><input type="hidden" name="id" value="'.(int)$edit.'"><div class="bvc-form-grid">';
        self::input('name','نام',$current['name']??'',true);self::input('base_url','Base URL',$current['base_url']??'',true);

        if($provider==='pasarguard'){
            self::select('auth_mode','روش ورود',['api_key'=>'API Key','password'=>'Username / Password'],$current['auth_mode']??'api_key');
            self::input('api_key','API Key','',false,'password');self::input('username','نام کاربری','',false);self::input('password','رمز','',false,'password');
            self::textarea('proxy_settings_json','Proxy Settings JSON',$current['proxy_settings_json']??'{"vless":{}}');
        }elseif($provider==='marzban'){
            self::input('username','نام کاربری مدیر','',false);self::input('password','رمز مدیر','',false,'password');
            self::textarea('proxies_json','Proxies JSON',$current['proxies_json']??'{}');self::textarea('inbounds_json','Inbounds JSON',$current['inbounds_json']??'{}');
        }elseif($provider==='hiddify'){
            self::input('subscription_base_url','Subscription Base URL (اختیاری)',$current['subscription_base_url']??'');
            self::input('api_key','Hiddify API Key (خالی = حفظ)','',false,'password');
            echo '<div class="bvc-note" style="grid-column:1/-1">API هدف: <code>/api/v2/admin/user/</code> • احراز هویت: <code>Hiddify-API-Key</code> • UUID هر مشتری در BlueVPN پایدار و deterministic است.</div>';
        }elseif($provider==='threexui'){
            self::input('subscription_base_url','Subscription Base URL (اختیاری)',$current['subscription_base_url']??'');
            self::select('auth_mode','روش ورود',['api_token'=>'API Token (پیشنهادی)','password'=>'Username / Password'],$current['auth_mode']??'api_token');
            self::input('api_token','API Token (خالی = حفظ)','',false,'password');
            self::input('username','نام کاربری (fallback)','');self::input('password','رمز (fallback)','',false,'password');
            echo '<div class="bvc-note" style="grid-column:1/-1">BlueVPN از Clients API و Inbounds API استفاده می‌کند؛ حجم، انقضا، IP/device limit و لینک Subscription در Sync/Repair کنترل می‌شوند.</div>';
        }else{
            ...
            self::input('api_key','API Key','',false,'password');self::input('username','نام کاربری','');self::input('password','رمز','',false,'password');
            ...
            if($edit)echo '<div class="bvc-note" style="grid-column:1/-1">GuardCore API 0.13: برای حساب‌های دارای TOTP، بعد از ذخیره Username/Password از دکمه «دریافت API Key با TOTP» استفاده کن.</div>';
        }

        echo '<label><input type="checkbox" name="verify_tls" value="1" '.checked(!isset($current['verify_tls'])||(int)($current['verify_tls']??1)===1,true,false).'> SSL Verify</label><label><input type="checkbox" name="active" value="1" '.checked(!isset($current['active'])||(int)($current['active']??1)===1,true,false).'> فعال</label></div>';
        submit_button($edit?'ذخیره تغییرات':'افزودن پنل','primary','submit',false);echo '</form></div>';

        if($provider==='guardcore'&&$edit){
            echo '<div class="bvc-grid"><div class="bvc-card"><h3>GuardCore API 0.13</h3><p>نسخه شناسایی‌شده: <code>'.self::esc($current['api_version']??'نامشخص').'</code></p>';
            ...
            echo '<div class="bvc-card"><h3>TOTP → API Key</h3><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_guardcore_bootstrap_key_'.$edit);echo '<input type="hidden" name="action" value="bluevpn_cc_guardcore_bootstrap_key"><input type="hidden" name="panel_id" value="'.$edit.'"><label>کد TOTP فعلی<input name="totp_code" inputmode="numeric" pattern="[0-9]{6,8}" maxlength="8" placeholder="123456"></label><p><button class="button">دریافت و ذخیره API Key</button></p></form></div></div>';
        }

        echo '<table class="widefat striped bvc-table"><tr><th>ID</th><th>نام</th><th>URL</th><th>وضعیت</th><th>آخرین تست</th><th>عملیات</th></tr>';
        foreach($rows as $x){
            $id=(int)$x['id'];$test=wp_nonce_url(admin_url('admin-post.php?action=bluevpn_cc_test_provider&provider='.$provider.'&id='.$id),'bluevpn_cc_test_provider_'.$provider.'_'.$id);
            $toggle=wp_nonce_url(admin_url('admin-post.php?action=bluevpn_cc_toggle_provider&provider='.$provider.'&id='.$id),'bluevpn_cc_toggle_provider_'.$provider.'_'.$id);
            $delete=wp_nonce_url(admin_url('admin-post.php?action=bluevpn_cc_delete_provider&provider='.$provider.'&id='.$id),'bluevpn_cc_delete_provider_'.$provider.'_'.$id);
            echo '<tr><td>'.$id.'</td><td>'.self::esc($x['name']).'</td><td><code>'.self::esc($x['base_url']).'</code></td><td>'.((int)$x['active']?'<span class="bvc-ok">فعال</span>':'<span class="bvc-bad">غیرفعال</span>').'</td>';
            echo '<td>'.((int)$x['last_test_ok']?'<span class="bvc-ok">✅</span>':'<span class="bvc-bad">'.($x['last_test_at']?'❌':'—').'</span>').' '.self::esc($x['last_test_message']??'').($x['last_test_at']?'<br><small>'.self::dt($x['last_test_at']).'</small>':'').'</td>';
            echo '<td><div class="bvc-actions"><a class="button" href="'.esc_url(self::url($provider==='pasarguard'?'panels':$provider,['edit'=>$id])).'">ویرایش</a><a class="button" href="'.esc_url($test).'">تست</a><a class="button" href="'.esc_url($toggle).'">'.((int)$x['active']?'غیرفعال':'فعال').'‌کردن</a><a class="button button-link-delete" href="'.esc_url($delete).'" onclick="return confirm(&quot;این پنل از BlueVPN حذف شود؟ مسیرهای محلی این پنل از پلن‌ها و mappingها پاک می‌شوند.&quot;)">حذف</a></div></td></tr>';
        }
        echo '</table>';
    }
    ...
    private static function tab_hiddify(): void { self::provider_tab('hiddify'); }
    private static function tab_threexui(): void { self::provider_tab('threexui'); }
    private static function tab_shahrah(): void { BlueVPN_Shahrah::render_admin_tab(); }
    ...
    private static function guardcore_api_dashboard(): void {
        ...
        foreach($panels as $panel){
            ...
            echo '<div style="display:flex;justify-content:space-between;gap:12px;flex-wrap:wrap;align-items:center"><div><strong>'.self::esc($panel['name']).'</strong> <code>#'.(int)$panel['id'].'</code><br><small>نشانی فنی: '.self::esc($catalog['version']??$panel['api_version']??'نامشخص').'</small></div>';
    ...
    private static function guardcore_assignments_panel(): void {
        ...
        foreach($byPanel as $group){
            ...
            foreach($users as $u){
                ...
                echo '<td>'.($updated?self::dt($updated):'—').'</td>';
                echo '<td>';
        ...
        if($inspectPanel>0&&$inspectUser!==''){
            ...
            if(empty($detail['ok']))echo '<p class="bvc-bad">'.self::esc($detail['message']??'خطا').'</p>';
                ...
                if($logs){echo '<h3>Usage Log</h3><table class="widefat striped bvc-table"><tr><th>زمان</th><th>مصرف</th></tr>';foreach(array_slice(array_reverse($logs),0,30) as $log)echo '<tr><td>'.self::dt($log['created_at']??'').'</td><td>'.self::fmt_bytes((int)($log['usage']??0)).'</td></tr>';echo '</table>';}
            }
            echo '</div>';
        }
        echo '</div>';
    ...
    private static function threexui_inbound_picker(array $panels,array $selectedByPanel=[]): void {
        echo '<div class="bvc-card" style="grid-column:1/-1;margin:0"><strong>Inboundهای 3x-ui</strong><p class="description">برای هر پنل می‌توانی Inboundهای مجاز را جدا انتخاب کنی. خالی = همه Inboundهای فعال همان پنل. لیست با «تست» پنل 3x-ui تازه می‌شود.</p>';
        $shown=false;
        foreach($panels as $panel){
            $pid=(int)$panel['id'];$items=BlueVPN_Utils::json_decode_array((string)($panel['inbounds_json']??''),[]);
            if(!$items)continue;$shown=true;$selected=array_map('intval',(array)($selectedByPanel[$pid]??[]));
            echo '<fieldset style="margin:10px 0;padding:10px;border:1px solid #dcdcde;border-radius:10px"><legend><strong>'.self::esc($panel['name']).' #'.$pid.'</strong></legend><div class="bvc-actions">';
            foreach($items as $item){if(!is_array($item)||empty($item['id']))continue;$iid=(int)$item['id'];$label=trim((string)($item['remark']??''))?:('Inbound #'.$iid);$meta=strtoupper((string)($item['protocol']??'')).((int)($item['port']??0)>0?' :'.(int)$item['port']:'');
                echo '<label class="bvc-access-chip"><input type="checkbox" name="threexui_inbound_ids_selected['.$pid.'][]" value="'.$iid.'" '.checked(in_array($iid,$selected,true),true,false).'><span>'.self::esc($label).'</span><small>'.self::esc($meta).'</small></label>';
            }
            echo '</div></fieldset>';
        }
        if(!$shown)echo '<div class="bvc-note">هنوز Catalog 3x-ui دریافت نشده است. ابتدا در صفحه 3x-ui روی «تست» بزن؛ بدون انتخاب هم BlueVPN همه Inboundهای فعال را خودکار استفاده می‌کند.</div>';
        echo '</div>';
    }
    ...
    private static function posted_threexui_inbounds(): array {
        $raw=$_POST['threexui_inbound_ids_selected']??[];if(!is_array($raw))return [];$out=[];
        foreach($raw as $panelId=>$ids){
            $pid=max(0,(int)$panelId);if($pid<=0||!is_array($ids))continue;
            foreach($ids as $value){$iid=max(0,(int)$value);if($iid>0&&!in_array($iid,$out[$pid]??[],true))$out[$pid][]=$iid;}
        }
        return $out;
    }
    ...
    public static function posted_provider_routes(): array {
        ...
        $xuiInboundMap=self::posted_threexui_inbounds();
        ...
        $hiIds=self::posted_multi_panel_ids('hiddify_panel_ids');
        $xuiIds=self::posted_multi_panel_ids('threexui_panel_ids');
        ...
        $routes=['pasarguard'=>[],'marzban'=>[],'shahrah'=>[],'guardcore'=>[],'hiddify'=>[],'threexui'=>[]];
        ...
        foreach($hiIds as $id){
            if(!$wpdb->get_var($wpdb->prepare('SELECT id FROM '.BlueVPN_DB::table('hiddify_panels').' WHERE id=%d',$id)))self::redirect('plans','یکی از سرورهای Hiddify انتخاب‌شده پیدا نشد.',true);
            $routes['hiddify'][]=['panel_id'=>$id];
        }
        foreach($xuiIds as $id){
            if(!$wpdb->get_var($wpdb->prepare('SELECT id FROM '.BlueVPN_DB::table('threexui_panels').' WHERE id=%d',$id)))self::redirect('plans','یکی از سرورهای 3x-ui انتخاب‌شده پیدا نشد.',true);
            $routes['threexui'][]=['panel_id'=>$id,'inbound_ids'=>array_values((array)($xuiInboundMap[$id]??[]))];
        }
        $seenSh=[];
    ...
    public static function provider_access_catalog(): void {
        ...
        if(!in_array($provider,['pasarguard','marzban','hiddify','threexui'],true)||$id<=0)wp_send_json_error(['message'=>'ابتدا یک پنل معتبر انتخاب کن.'],400);
    ...
    private static function tab_plans(): void {
        global $wpdb;
        $p=BlueVPN_DB::table('plans');$pg=BlueVPN_DB::table('pasarguard_panels');$mz=BlueVPN_DB::table('marzban_panels');$sh=BlueVPN_DB::table('shahrah_panels');$gc=BlueVPN_DB::table('guardcore_panels');$hi=BlueVPN_DB::table('hiddify_panels');$xui=BlueVPN_DB::table('threexui_panels');
        ...
        $hiRows=$wpdb->get_results("SELECT id,name,active FROM {$hi} ORDER BY name,id",ARRAY_A)?:[];
        $xuiRows=$wpdb->get_results("SELECT id,name,active,inbounds_json FROM {$xui} ORDER BY name,id",ARRAY_A)?:[];
        $shCatalog=class_exists('BlueVPN_Shahrah')?BlueVPN_Shahrah::plan_catalog(true):[];
        $rows=$wpdb->get_results("SELECT p.*,pg.name pg_name,mz.name mz_name,sh.name shahrah_name,gc.name gc_name,gc.auth_mode gc_auth_mode FROM {$p} p LEFT JOIN {$pg} pg ON pg.id=p.panel_id LEFT JOIN {$mz} mz ON mz.id=p.marzban_panel_id LEFT JOIN {$sh} sh ON sh.id=p.shahrah_panel_id LEFT JOIN {$gc} gc ON gc.id=p.guardcore_panel_id WHERE p.deleted=0 ORDER BY p.sort_order,p.id",ARRAY_A);
        ...
        $shahrah_select=function(string $name,int $currentPanel=0,string $currentSlug='') use($shCatalog){
            ...
            foreach($shCatalog as $item){
                ...
                $caption=(string)$item['panel_name'].' — '.(string)$item['name'].' ['.(string)$item['slug'].']';
                echo '<option value="'.esc_attr($value).'" '.selected($current,$value,false).'>'.esc_html($caption).'</option>';
            }
        ...
        echo '<label>سرورهای هیدیفای';$multi_select('hiddify_panel_ids',$hiRows,[]);echo '</label>';
        echo '<label>سرورهای 3x-ui';$multi_select('threexui_panel_ids',$xuiRows,[]);echo '</label>';
        ...
        echo '</div>';self::threexui_inbound_picker($xuiRows,[]);BlueVPN_Subscription_Sources::render_plan_picker([]);echo '<p class="description">برای چند سرور، دسترسی‌های فعال هر سرور به‌صورت خودکار استفاده می‌شوند. تنظیمات قدیمی اولین سرور برای پلن‌های قبلی همچنان پشتیبانی می‌شود.</p></div>';
        ...
        foreach($rows as $x){
            $id=(int)$x['id'];
            $routes=class_exists('BlueVPN_Providers')?BlueVPN_Providers::plan_provider_routes($x):['pasarguard'=>[],'marzban'=>[],'shahrah'=>[],'guardcore'=>[],'hiddify'=>[],'threexui'=>[]];
            ...
            $hiSelectedPanels=array_values(array_map(static fn($r)=>(int)($r['panel_id']??0),(array)$routes['hiddify']));
            $xuiSelectedPanels=array_values(array_map(static fn($r)=>(int)($r['panel_id']??0),(array)$routes['threexui']));
            $xuiSelectedInbounds=[];foreach((array)$routes['threexui'] as $xr){$pid=(int)($xr['panel_id']??0);if($pid>0)$xuiSelectedInbounds[$pid]=array_values(array_map('intval',(array)($xr['inbound_ids']??[])));}
            ...
            $trafficMode=(string)($x['traffic_mode']??'provider_reported');
            $gatewayReplicas=max(1,min(3,(int)($x['gateway_replica_count']??2)));
            $sourceSelected=BlueVPN_Subscription_Sources::source_ids_for_plan($x);
            ...
            echo '<span class="bvc-provider-pill '.($hiSelectedPanels?'is-on':'is-empty').'">هیدیفای: '.count($hiSelectedPanels).'</span>';
            echo '<span class="bvc-provider-pill '.($xuiSelectedPanels?'is-on':'is-empty').'">3x-ui: '.count($xuiSelectedPanels).'</span>';
            ...
            echo '<label>سرورهای هیدیفای';$multi_select('hiddify_panel_ids',$hiRows,$hiSelectedPanels);echo '</label>';
            echo '<label>سرورهای 3x-ui';$multi_select('threexui_panel_ids',$xuiRows,$xuiSelectedPanels);echo '</label>';
            ...
            echo '</div>';self::threexui_inbound_picker($xuiRows,$xuiSelectedInbounds);BlueVPN_Subscription_Sources::render_plan_picker($sourceSelected);
            ...
            echo '<label>مرجع حجم<select name="traffic_mode"><option value="provider_reported" '.selected($trafficMode,'provider_reported',false).'>Provider reported</option><option value="gateway_metered" '.selected($trafficMode,'gateway_metered',false).'>BlueVPN Gateway</option></select></label>';echo '<label>Gateway replicas<input type="number" name="gateway_replica_count" min="1" max="3" value="'.$gatewayReplicas.'"></label>';
            echo '</div>';self::threexui_inbound_picker($xuiRows,$xuiSelectedInbounds);BlueVPN_Subscription_Sources::render_plan_picker($sourceSelected);echo '<p class="description">دسترسی‌ها و سرویس‌های فعال هر سرور چندگانه خودکار انتخاب می‌شوند.</p><div class="bvc-form-grid">';
    ...
    private static function tab_blupal(): void {
        global $wpdb;$t=BlueVPN_DB::table('payment_settings');$cfg=$wpdb->get_row("SELECT * FROM {$t} WHERE id=1",ARRAY_A)?:[];$orders=BlueVPN_DB::table('orders');
        $pending=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$orders} WHERE status IN ('created','creating_invoice','pending','paid','paid_needs_sync','partial_needs_sync')");
        $paid=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$orders} WHERE status IN ('activated','paid_needs_sync','partial_needs_sync')");
        $failed=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$orders} WHERE status IN ('invoice_failed','failed','expired','canceled','amount_mismatch')");
        $attempts=BlueVPN_DB::table('provisioning_attempts');$provisionFailed=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$attempts} WHERE status='failed'");
        echo '<div class="bvc-grid"><div class="bvc-card bvc-kpi"><span>نیازمند پیگیری</span><strong>'.$pending.'</strong></div><div class="bvc-card bvc-kpi"><span>پرداخت/فعال‌سازی موفق</span><strong>'.$paid.'</strong></div><div class="bvc-card bvc-kpi"><span>پرداخت ناموفق</span><strong>'.$failed.'</strong></div><div class="bvc-card bvc-kpi"><span>Provision ناموفق</span><strong>'.$provisionFailed.'</strong></div></div>';
        echo '<div class="bvc-grid"><div class="bvc-card"><h3>Webhook بلوپال</h3><div class="bvc-code">'.self::esc(BlueVPN_Payments::webhook_url()).'</div><p><small>این URL را در تنظیمات API Key بلوپال ثبت کن. Webhook فقط Trigger است؛ BlueVPN قبل از فعال‌سازی، وضعیت فاکتور را دوباره Server-to-Server تأیید می‌کند.</small></p></div><div class="bvc-card"><h3>چرخه فعال‌سازی</h3><p>Invoice → PAID → Verify → Provision Providerها → Entitlement → SMS. هر مرحله Idempotent و قابل Retry ثبت می‌شود.</p></div></div>';
        echo '<div class="bvc-card"><h2>تنظیمات بلوپال</h2><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_save_payment');echo '<input type="hidden" name="action" value="bluevpn_cc_save_payment"><div class="bvc-form-grid">';self::input('base_url','Base URL',$cfg['base_url']??'https://blupal.net/api',true);self::input('api_key','API Key (خالی = حفظ فعلی)','',false,'password');self::input('card_number','شماره کارت مقصد اختیاری',$cfg['card_number']??'');echo '<label><input type="checkbox" name="active" value="1" '.checked((int)($cfg['active']??0),1,false).'> فعال</label></div>';submit_button('ذخیره بلوپال','primary','submit',false);echo '</form>';
        echo '<div class="bvc-note" style="margin-top:16px"><strong>آدرس‌های لازم در BluPal</strong><br>Webhook URL<br><code style="direction:ltr;display:inline-block;margin:6px 0">'.esc_html(BlueVPN_Payments::webhook_url()).'</code><br>Callback page<br><code style="direction:ltr;display:inline-block;margin:6px 0">'.esc_html(BlueVPN_Payments::callback_url()).'</code><br><small>Webhook فقط POST می‌پذیرد. Callback یک صفحه عمومی HTML است و نتیجه را دوباره از API بلوپال Verify می‌کند.</small></div></div>';
        $events=BlueVPN_DB::table('payment_events');$rows=$wpdb->get_results("SELECT e.*,o.order_code FROM {$events} e LEFT JOIN {$orders} o ON o.id=e.order_id ORDER BY e.id DESC LIMIT 30",ARRAY_A)?:[];
        echo '<div class="bvc-card"><h2>رویدادهای پرداخت تأییدشده</h2><table class="widefat striped bvc-table"><tr><th>سفارش</th><th>Invoice</th><th>رویداد</th><th>وضعیت</th><th>مبلغ ریال</th><th>Verified</th><th>زمان</th></tr>';foreach($rows as $x)echo '<tr><td>'.self::esc($x['order_code']?:$x['order_id']).'</td><td><code>'.self::esc($x['provider_invoice_id']).'</code></td><td>'.self::esc($x['event_type']).'</td><td>'.self::esc($x['status']).'</td><td>'.number_format((int)$x['amount_rial']).'</td><td>'.((int)$x['verified']?'✅':'—').'</td><td>'.self::dt($x['created_at']).'</td></tr>';echo '</table></div>';
    }
    ...
    private static function tab_customers(): void {
        ...
        $t=BlueVPN_DB::table('customers');$q=sanitize_text_field(wp_unslash($_GET['q']??''));$where='1=1';if($q!==''){$like='%'.$wpdb->esc_like($q).'%';$where=$wpdb->prepare('(email LIKE %s OR phone LIKE %s OR pg_username LIKE %s OR marzban_username LIKE %s OR guardcore_username LIKE %s)',$like,$like,$like,$like,$like);} $rows=$wpdb->get_results("SELECT * FROM {$t} WHERE {$where} ORDER BY id DESC LIMIT 250",ARRAY_A);
        ...
        echo '<div class="bvc-card" id="bvc-provider-repair-card"><div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap"><div><h2 style="margin-bottom:6px">🧩 همگام‌سازی اشتراک‌های گمشده Provider</h2><div class="bvc-note">کاربران دارای اشتراک فعال را روی همه مسیرهای پولی بررسی می‌کند: PasarGuard، Marzban و GuardCore، همچنین Shahrah، Sourceهای ثابت و Gateway Metered. اشتراک گمشده با اعتبار فعلی بازیابی می‌شود، دسترسی Group/Inbound/Service طبق پلن دوباره Sync می‌شود، Shahrah در Repair هرگز Renew نمی‌شود و فقط Service موجود را پیدا/Attach یا در صورت نبود واقعی Create می‌کند؛ Sourceهای ثابت Snapshot را بازسازی می‌کنند و Gateway sessionهای گمشده را دوباره جایگذاری می‌کند. این عملیات تاریخ اشتراک را تمدید نمی‌کند؛ تاریخ اشتراک WordPress بدون تغییر می‌ماند و مصرف را Reset نمی‌کند.</div></div><div class="bvc-kpi"><span>کاربر قابل اسکن</span><strong>'.number_format($repairable).'</strong></div></div>';
        ...
        if(!empty($lastRepair['finished_at']))echo '<p style="margin:12px 0 0"><small>آخرین اجرا: '.self::esc($lastRepair['finished_at']).' • اسکن '.number_format((int)($lastRepair['processed']??0)).' • ساخته‌شده '.number_format((int)($lastRepair['created']??0)).' • اتصال بازیابی‌شده '.number_format((int)($lastRepair['attached']??0)).' • Sync مجدد '.number_format((int)($lastRepair['resynced']??0)).' • بدون تغییر '.number_format((int)($lastRepair['existing']??0)).' • خطا '.number_format((int)($lastRepair['error_count']??0)).'</small></p>';
        echo '</div>';
        echo '<script>(function(){const btn=document.getElementById("bvc-provider-repair-start");if(!btn)return;const box=document.getElementById("bvc-provider-repair-progress"),bar=document.getElementById("bvc-provider-repair-bar"),txt=document.getElementById("bvc-provider-repair-text"),errs=document.getElementById("bvc-provider-repair-errors");let stopped=false;async function step(cursor,job){if(stopped)return;const body=new URLSearchParams();body.set("action","bluevpn_cc_repair_missing_provider_subscriptions");body.set("_ajax_nonce",'.wp_json_encode($repairNonce).');body.set("cursor",String(cursor||0));if(job)body.set("job_id",job);try{const r=await fetch('.wp_json_encode(admin_url('admin-ajax.php')).',{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"},body:body.toString()});const raw=await r.text();let j=null;try{j=JSON.parse(raw);}catch(_){throw new Error("HTTP "+r.status+" • پاسخ غیر JSON از سرور");}if(!r.ok||!j||!j.success)throw new Error((j&&j.data&&j.data.message)||("HTTP "+r.status+" • پاسخ نامعتبر سرور"));const d=j.data||{},s=d.summary||{};const total=Math.max(1,Number(s.total||0)),processed=Number(s.processed||0);bar.style.width=Math.min(100,Math.round(processed*100/total))+"%";txt.textContent="اسکن "+processed+" از "+Number(s.total||0)+" • ساخته‌شده "+Number(s.created||0)+" • اتصال بازیابی‌شده "+Number(s.attached||0)+" • Sync مجدد "+Number(s.resynced||0)+" • موجود "+Number(s.existing||0)+" • تعویق شاهراه "+Number(s.deferred||0)+" • خطا "+Number(s.error_count||0);if(Array.isArray(s.errors)&&s.errors.length)errs.textContent=s.errors.slice(-4).join(" | ");if(d.done){stopped=true;btn.disabled=false;btn.textContent="اسکن دوباره";txt.textContent="✅ همگام‌سازی کامل شد — "+txt.textContent;return;}const recent=Array.isArray(s.errors)?s.errors.slice(-4).join(" "):"";const wait=/شاهراه.*(?:HTTP 5\d\d|پاسخ معتبر JSON|خطای داخلی)/.test(recent)?2500:650;setTimeout(()=>step(Number(d.next_cursor||0),String(d.job_id||job||"")),wait);}catch(e){txt.textContent="⚠️ "+e.message+" — تلاش مجدد…";setTimeout(()=>step(cursor,job),2500);}}btn.addEventListener("click",function(){if(!confirm("تمام کاربران دارای اشتراک فعال روی همه مسیرهای پولی شامل PasarGuard، Marzban، GuardCore، Shahrah، Sourceهای ثابت و Gateway بررسی شوند و اشتراک‌های گمشده بدون تمدید entitlement بازیابی شوند؟\n\nاین عملیات تاریخ اشتراک WordPress را تمدید نمی‌کند."))return;stopped=false;btn.disabled=true;box.style.display="block";errs.textContent="";bar.style.width="0";txt.textContent="شروع اسکن…";step(0,"");});})();</script>';
    ...
    private static function customer_detail(int $customerId): void {
        ...
        $currentLimitGb=((int)$c['data_limit_bytes'])>0?round((int)$c['data_limit_bytes']/(1024*1024*1024),2):0;
        $currentExpireFa=BlueVPN_Utils::tehran_date_fa($c['subscription_expire']??null);
        echo '<div class="bvc-card"><h2>✏️ ویرایش مستقیم اعتبار کاربر</h2><div class="bvc-note">این فرم <strong>تمدید تجمعی نیست</strong>؛ تاریخ و حجم نهایی را دقیقاً روی مقدار واردشده Set می‌کند، حتی اگر پلن همین حالا فعال باشد. پلن و مسیر Provider عوض نمی‌شوند.</div><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_update_customer_entitlement_'.$customerId);echo '<input type="hidden" name="action" value="bluevpn_cc_update_customer_entitlement"><input type="hidden" name="customer_id" value="'.$customerId.'"><div class="bvc-form-grid"><label>تاریخ انقضای نهایی شمسی<input type="text" name="absolute_expire_date" value="'.esc_attr($currentExpireFa).'" placeholder="مثال: 1405/07/15" required><small>مقدار نهایی؛ به تاریخ قبلی اضافه نمی‌شود.</small></label><label>حجم کل نهایی (GB)<input type="number" min="0" max="100000" step="0.1" name="absolute_data_limit_gb" value="'.esc_attr(rtrim(rtrim(number_format($currentLimitGb,2,'.',''),'0'),'.')).'" required><small>۰ = نامحدود. مصرف فعلی Reset نمی‌شود.</small></label></div>';submit_button('ذخیره زمان و حجم','primary','submit',false);echo '</form></div>';
        $routeAudit=BlueVPN_Providers::subscription_route_audit($customerId);
        echo '<div class="bvc-card"><h2>ممیزی مسیرهای اشتراک</h2>';
        echo '<p><strong>'.(int)($routeAudit['healthy']??0).'</strong> مسیر سالم از <strong>'.(int)($routeAudit['expected']??0).'</strong> مسیر مورد انتظار • کل کانفیگ: <strong>'.(int)($routeAudit['total_configs']??0).'</strong></p>';
        if(empty($routeAudit['routes'])){
            echo '<div class="bvc-note">برای این مشتری هنوز مسیر پولی قابل ممیزی ثبت نشده است.</div>';
        }else{
            echo '<table class="widefat striped bvc-table"><tr><th>مسیر</th><th>تأمین‌کننده</th><th>اتصال</th><th>کانفیگ</th><th>وضعیت</th><th>خطا</th></tr>';
            foreach((array)$routeAudit['routes'] as $rr){
                $status=(string)($rr['status']??'missing_link');
                $label=$status==='healthy'?'✅ سالم':($status==='empty_snapshot'?'⚠️ بدون کانفیگ':'❌ اتصال گمشده');
                echo '<tr><td><code>'.self::esc($rr['key']??'').'</code></td><td>'.self::esc($rr['provider']??'').'</td><td>#'.(int)($rr['panel_id']??0).'</td><td>'.(int)($rr['config_count']??0).'</td><td>'.$label.'</td><td><small class="bvc-bad">'.self::esc(mb_substr((string)($rr['error']??''),0,160)).'</small></td></tr>';
            }
            echo '</table>';
        }
        echo '<p><small>اگر یکی از مسیرهای انتخاب‌شده پلن گم باشد، Snapshot قبل از تحویل به کاربر Repair امن را اجرا می‌کند و سپس دوباره همه مسیرها را جمع می‌کند.</small></p></div>';
        echo '<div class="bvc-grid"><div class="bvc-card"><h2>وضعیت حساب</h2><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_set_customer_status_'.$customerId);echo '<input type="hidden" name="action" value="bluevpn_cc_set_customer_status"><input type="hidden" name="customer_id" value="'.$customerId.'"><label><input type="checkbox" name="active" value="1" '.checked((int)$c['active'],1,false).'> حساب فعال باشد</label><br><br><label><input type="checkbox" name="beta_tester" value="1" '.checked((int)($c['beta_tester']??0),1,false).'> 🧪 آزمایش‌کننده نسخه آزمایشی — نسخه‌های آزمایشی را دریافت کند</label><p><button class="button button-primary">ذخیره وضعیت</button></p></form><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_logout_customer_'.$customerId);echo '<input type="hidden" name="action" value="bluevpn_cc_logout_customer"><input type="hidden" name="customer_id" value="'.$customerId.'"><button class="button">خروج اجباری از همه نشست‌ها</button></form></div>';
        echo '<div class="bvc-card"><h2>پلن / تمدید دستی</h2><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_manual_activate');echo '<input type="hidden" name="action" value="bluevpn_cc_manual_activate"><input type="hidden" name="customer_id" value="'.$customerId.'"><div class="bvc-form-grid"><label>پلن<select name="plan_id" required><option value="">انتخاب…</option>';foreach($plans as $p)echo '<option value="'.(int)$p['id'].'" '.selected((int)$c['plan_id'],(int)$p['id'],false).'>'.self::esc($p['title']).' — '.(int)$p['duration_days'].' روز — '.(int)$p['data_limit_gb'].' GB</option>';echo '</select></label><label>مدت دلخواه (روز)<input type="number" min="1" max="3650" step="1" name="custom_duration_days" placeholder="خالی = مدت پلن"></label><label>حجم دلخواه (GB)<input type="number" min="0" max="100000" step="0.1" name="custom_data_limit_gb" placeholder="خالی = حجم پلن"><small>۰ = نامحدود</small></label></div><p><button class="button button-primary">فعال‌سازی / تمدید / تغییر پلن</button></p></form><hr><p><strong>ترمیم Provider بدون تمدید</strong></p><p><small>همه مسیرهای پولی این پلن را بررسی می‌کند: PasarGuard، Marzban، GuardCore، Shahrah، Sourceهای ثابت و Gateway. اشتراک گمشده بدون تمدید entitlement بازیابی می‌شود و دسترسی‌های ذخیره‌شده پلن دوباره Sync می‌شوند.</small></p><a class="button" href="'.esc_url(wp_nonce_url(admin_url('admin-post.php?action=bluevpn_cc_repair_customer_providers&customer_id='.$customerId),'bluevpn_cc_repair_customer_providers_'.$customerId)).'">ترمیم اشتراک Provider</a></div></div>';
        ...
        foreach($sessions as $ss){$sid=(int)$ss['id'];$live=empty($ss['revoked_at'])&&!empty($ss['expires_at'])&&strtotime((string)$ss['expires_at'].' UTC')>time();echo '<tr><td>#'.$sid.'</td><td><code>'.self::esc($ss['device_id']).'</code></td><td>'.self::dt($ss['created_at']).'</td><td>'.self::dt($ss['last_seen_at']).'</td><td>'.self::dt($ss['expires_at']).'</td><td class="'.($live?'bvc-ok':'bvc-warn').'">'.($live?'فعال':'باطل/منقضی').'</td><td>';if($live){echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_revoke_session_'.$customerId.'_'.$sid);echo '<input type="hidden" name="action" value="bluevpn_cc_revoke_session"><input type="hidden" name="customer_id" value="'.$customerId.'"><input type="hidden" name="session_id" value="'.$sid.'"><button class="button">ابطال نشست</button></form>';}else echo '—';echo '</td></tr>';}
        echo '</table></div>';
        echo '<div class="bvc-card"><h2>آخرین سفارش‌ها</h2><table class="widefat striped bvc-table"><tr><th>فاکتور</th><th>پلن</th><th>مبلغ</th><th>وضعیت</th><th>ایجاد</th><th>فعال‌سازی</th></tr>';foreach($orders as $o)echo '<tr><td><code>'.self::esc($o['order_code']).'</code></td><td>'.self::esc($o['plan_title']).'</td><td>'.number_format((int)$o['amount_toman']).'</td><td>'.self::esc($o['status']).'</td><td>'.self::dt($o['created_at']).'</td><td>'.self::dt($o['activated_at']).'</td></tr>';echo '</table></div>';
    }
    ...
    private static function tab_orders(): void {
        ...
        echo '<div class="bvc-card"><h2>مدیریت پرداخت و فعال‌سازی</h2><p>پرداخت و Provision دو مرحله مستقل ثبت می‌شوند. اگر پرداخت تأیید شده ولی Provider ناقص باشد، بدون ساخت فاکتور جدید می‌توان Provision را Retry کرد.</p></div>';
        echo '<table class="widefat striped bvc-table"><tr><th>سفارش</th><th>کاربر</th><th>پلن</th><th>مبلغ</th><th>وضعیت</th><th>BluPal Invoice</th><th>تراکنش</th><th>پرداخت‌کننده</th><th>خطا</th><th>زمان</th><th>عملیات</th></tr>';
        foreach($rows as $x){echo '<tr><td><code>'.self::esc($x['order_code']).'</code></td><td>'.self::esc($x['phone']?:$x['email']).'</td><td>'.self::esc($x['plan_title']).'</td><td>'.number_format((int)$x['amount_toman']).' تومان<br><small>'.number_format((int)($x['final_amount_rial']??0)).' ریال نهایی</small></td><td>'.self::esc($x['status']).'</td><td><code>'.self::esc($x['payment_id']).'</code><br><small>'.self::esc($x['payment_mode']??'').'</small></td><td><code>'.self::esc($x['transaction_id']??'').'</code></td><td>'.self::esc($x['payer_name']??'').'<br><small>'.self::esc($x['payer_bank_name']??'').'</small></td><td><small class="bvc-bad">'.self::esc($x['activation_error']).'</small></td><td>'.self::dt($x['created_at']).'</td><td>';
            if(in_array((string)$x['status'],['paid','paid_needs_sync','partial_needs_sync'],true)){echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_retry_order_provision_'.$x['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_retry_order_provision"><input type="hidden" name="order_id" value="'.esc_attr($x['id']).'"><button class="button button-primary">تلاش دوباره برای فعال‌سازی</button></form>';}
            echo '</td></tr>';}
        echo '</table>';
    }
    private static function tab_android(): void {
        global $wpdb;
        ...
        echo '<div class="bvc-note"><strong>کانال‌های انتشار فعال هستند.</strong> هر ساخت جدید در مخزن کد به‌صورت پیش‌فرض فقط وارد کانال <strong>آزمایشی</strong> می‌شود. کاربران عادی تا زمانی که روی «انتشار رسمی» نزنید همان نسخه رسمی قبلی را می‌گیرند.</div>';
        echo '<div class="bvc-grid">';
        echo '<div class="bvc-card bvc-kpi"><span>نسخه رسمی</span><strong>'.self::esc($stable['version']??'—').'</strong><small>'.($stable?'ساخت شماره '.(int)$stable['build_number']:'هنوز انتشار رسمی ندارد').'</small></div>';
        echo '<div class="bvc-card bvc-kpi"><span>آخرین نسخه آزمایشی فعال</span><strong>'.self::esc($beta['version']??'—').'</strong><small>'.($beta?'فقط برای آزمایش‌کنندگان':'نسخه آزمایشی فعالی وجود ندارد').'</small></div>';
        echo '<div class="bvc-card bvc-kpi"><span>آزمایش‌کننده نسخه آزمایشی</span><strong>'.number_format($betaCount).'</strong><small>از بخش کاربران قابل تعیین است</small></div>';
        echo '<div class="bvc-card"><h3>همگام‌سازی مخزن کد</h3><p class="'.esc_attr($statusClass).'">'.self::esc((string)($status['message']??'در انتظار اولین همگام‌سازی')).'</p><small>'.($last?'آخرین بررسی: '.self::dt($last):'هنوز اجرا نشده').'</small></div>';
        echo '</div>';

        echo '<div class="bvc-card"><h2>نسخه‌ها و کانال انتشار</h2>';
        if(!$releases){echo '<p class="bvc-warn">هنوز نسخه‌ای ثبت نشده است. اگر نسخه رسمی فعلی در تنظیمات قبلی وجود داشته باشد، با ارتقای Schema به‌عنوان نسخه رسمی اولیه می‌شود.</p>';}
        else{
            echo '<table class="widefat striped bvc-table"><tr><th>نسخه</th><th>کانال</th><th>ساخت / شناسه تغییر</th><th>اجبار</th><th>تاریخ</th><th>عملیات</th></tr>';
            foreach($releases as $r){
                $state=(string)$r['state'];
                $label=['stable'=>'🟢 رسمی','beta'=>'🟡 آزمایشی','stopped'=>'⛔ آزمایشی متوقف','archived'=>'⚪ آرشیو'][$state]??$state;
                echo '<tr><td><strong>'.self::esc($r['version']).'</strong><br><small>کد '.(int)$r['version_code'].'</small></td><td>'.self::esc($label).'</td><td>#'.(int)$r['build_number'].'<br><code>'.self::esc(substr((string)$r['commit_sha'],0,12)?:'—').'</code></td><td>'.(!empty($r['force_update'])?'<span class="bvc-bad">اجباری</span>':'اختیاری').'</td><td>'.self::dt($r['release_published_at']).'</td><td><div class="bvc-actions">';
                if(!empty($r['release_url']))echo '<a class="button" target="_blank" rel="noopener" href="'.esc_url($r['release_url']).'">GitHub</a>';
                if($state!=='stable'){
                    echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_release_promote_'.(int)$r['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_release_promote"><input type="hidden" name="release_id" value="'.(int)$r['id'].'"><input type="hidden" name="scope" value="all"><button class="button button-primary">انتشار رسمی اندروید و ویندوز</button></form>';
                    echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_release_promote_'.(int)$r['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_release_promote"><input type="hidden" name="release_id" value="'.(int)$r['id'].'"><input type="hidden" name="scope" value="android_only"><button class="button">فقط اندروید</button></form>';
                }
                if($state==='beta'){echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_release_stop_'.(int)$r['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_release_stop"><input type="hidden" name="release_id" value="'.(int)$r['id'].'"><button class="button">توقف نسخه آزمایشی</button></form>';}
                elseif(in_array($state,['stopped','archived'],true)){echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_release_resume_'.(int)$r['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_release_resume"><input type="hidden" name="release_id" value="'.(int)$r['id'].'"><button class="button">فعال‌سازی نسخه آزمایشی</button></form>';}
        ...
        echo '<div class="bvc-card"><h2>سیاست بروزرسانی</h2><p>ساخته‌شدن فایل نصب اندروید با انتشار عمومی یکی نیست. آزمایش‌کنندگان همان بررسی و دانلود خودکار نسخه رسمی را دارند؛ به‌روزرسانی خودکار هر کانال مستقل است. Beta اجبار فقط روی همان Release آزمایشی اثر می‌گذارد و اجبار نسخه Stable برای همه کاربران است.</p><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';
        ...
        self::input('owner','مالک مخزن',$cfg['owner'],true);self::input('repo','نام مخزن',$cfg['repo'],true);self::input('minimum_version','حداقل نسخه قابل استفاده',$s['minimum_version'],true);self::input('support_url','لینک پشتیبانی',$s['support_url']);self::input('title_override','عنوان آپدیت (اختیاری)',$cfg['title_override']);self::textarea('message_override','متن آپدیت (اختیاری)',$cfg['message_override']);
        echo '<label><input type="checkbox" name="app_auto_sync" value="1" '.checked(!empty($cfg['auto_sync']),true,false).'> همگام‌سازی خودکار نسخه‌های مخزن</label>';
        echo '<label><input type="checkbox" name="auto_update_stable" value="1" '.checked(!empty($s['auto_update_stable']),true,false).'> دانلود خودکار نسخه رسمی برای کاربران عادی</label>';
        echo '<label><input type="checkbox" name="auto_update_beta" value="1" '.checked(!empty($s['auto_update_beta']),true,false).'> 🧪 دانلود خودکار نسخه آزمایشی برای آزمایش‌کنندگان</label>';
        ...
        echo '<div class="bvc-card"><h2>اتوماسیون انتشار</h2><div class="bvc-grid"><div><strong>① ساخت</strong><p>سامانه انتشار فایل نصب اندروید را می‌سازد.</p></div><div><strong>② آزمایشی</strong><p>WordPress نسخه جدید را خودکار Beta ثبت می‌کند؛ فقط آزمایش‌کنندگان می‌بینند.</p></div><div><strong>③ رسمی</strong><p>دکمه اصلی «انتشار رسمی اندروید و ویندوز» نسخه هم‌نام را روی هر دو پلتفرم Stable می‌کند؛ گزینه «فقط اندروید» برای انتشار مستقل باقی مانده است.</p></div></div>';
        echo '<p><strong>نشانی فنی:</strong></p><div class="bvc-code">'.self::esc(untrailingslashit(home_url('/')).'/api/v1/mobile/config').'</div>';
        echo '<div class="bvc-actions" style="margin-top:12px"><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_sync_app_release');echo '<input type="hidden" name="action" value="bluevpn_cc_sync_app_release">';submit_button('همگام‌سازی همین حالا','secondary','submit',false);echo '</form><a class="button" href="'.esc_url(self::url('customers')).'">مدیریت آزمایش‌کنندگان</a></div></div>';
        echo '<div class="bvc-note"><strong>نکته راه‌اندازی اولیه:</strong> نسخه‌های قبل از 4.3.1 هنگام درخواست تنظیمات، توکن حساب را ارسال نمی‌کنند. بنابراین اولین آزمایش‌کنندگان باید APK 4.3.1 را یک‌بار دستی نصب کنند؛ از 4.3.1 به بعد دریافت Beta کاملاً خودکار و بر اساس حساب پنل است.</div>';
    }

    private static function tab_windows(): void {
        global $wpdb;
        $betaCount=(int)$wpdb->get_var('SELECT COUNT(*) FROM '.BlueVPN_DB::table('customers').' WHERE active=1 AND beta_tester=1');
    ...
    private static function tab_app(): void {
        echo '<div class="bvc-card"><h2>انتشار برنامه‌ها</h2><p>تنظیمات اندروید و ویندوز از هم جدا شده‌اند تا مدیریت نسخه‌ها ساده‌تر باشد.</p><div class="bvc-actions"><a class="button button-primary" href="'.esc_url(self::url('android')).'">تنظیمات اندروید</a><a class="button" href="'.esc_url(self::url('windows')).'">تنظیمات ویندوز</a></div></div>';
    }
    ...
    private static function render_windows_release_management(int $betaCount): void {
        ...
        $statusClass=in_array((string)($status['status']??''),['synced','up_to_date'],true)?'bvc-ok':(((string)($status['status']??'')==='never')?'bvc-warn':'bvc-bad');
        echo '<div class="bvc-note"><strong>کانال‌های انتشار ویندوز</strong> مستقل از اندروید مدیریت می‌شود. هر فایل نصب جدید ابتدا <strong>آزمایشی</strong> است؛ انتشار رسمی فقط با تصمیم مدیر انجام می‌شود و ساخت دوباره لازم ندارد.</div>';
        echo '<div class="bvc-grid">';
        echo '<div class="bvc-card bvc-kpi"><span>نسخه رسمی ویندوز</span><strong>'.self::esc($stable['version']??'—').'</strong><small>'.($stable?'برای همه کاربران ویندوز':'هنوز نسخه رسمی Windows ندارد').'</small></div>';
        echo '<div class="bvc-card bvc-kpi"><span>نسخه آزمایشی فعال ویندوز</span><strong>'.self::esc($beta['version']??'—').'</strong><small>'.($beta?'فقط آزمایش‌کنندگان':'نسخه آزمایشی فعالی وجود ندارد').'</small></div>';
        echo '<div class="bvc-card bvc-kpi"><span>آزمایش‌کننده مشترک</span><strong>'.number_format($betaCount).'</strong><small>همان پرچم حساب برای اندروید و ویندوز</small></div>';
        echo '<div class="bvc-card"><h3>همگام‌سازی ویندوز</h3><p class="'.esc_attr($statusClass).'">'.self::esc((string)($status['message']??'در انتظار اولین همگام‌سازی')).'</p><small>'.($last?'آخرین بررسی: '.self::dt($last):'هنوز اجرا نشده').'</small></div>';
        echo '</div>';

        echo '<div class="bvc-card"><h2>نسخه‌های ویندوز و کانال انتشار</h2>';
        if(!$releases){echo '<p class="bvc-warn">هنوز فایل نصب ویندوز در پنل ثبت نشده است. «همگام‌سازی ویندوز» را بزن؛ نسخه‌های ویندوز از مخزن دریافت و نسخه‌های جدید به‌صورت Beta ثبت می‌شوند.</p>';}
        else{
            echo '<table class="widefat striped bvc-table"><tr><th>نسخه</th><th>کانال</th><th>فایل نصب</th><th>اجبار</th><th>تاریخ</th><th>عملیات</th></tr>';
            foreach($releases as $r){
                $state=(string)$r['state'];$label=['stable'=>'🟢 رسمی','beta'=>'🟡 آزمایشی','stopped'=>'⛔ آزمایشی متوقف','archived'=>'⚪ آرشیو'][$state]??$state;
                $meta=is_array($r['asset_meta']??null)?$r['asset_meta']:[];$x64=$meta['installer_x64']??[];$arm=$meta['installer_arm64']??[];
                echo '<tr><td><strong>'.self::esc($r['version']).'</strong><br><small>کد '.(int)$r['version_code'].'</small></td><td>'.self::esc($label).'</td><td><small>x64 '.(!empty($r['installer_x64_url'])?'✅':'❌').' '.(!empty($x64['sha256'])?'اثر انگشت ✅':'اثر انگشت ⚠️').'<br>ARM64 '.(!empty($r['installer_arm64_url'])?'✅':'❌').' '.(!empty($arm['sha256'])?'اثر انگشت ✅':'اثر انگشت ⚠️').'</small></td><td>'.(!empty($r['force_update'])?'<span class="bvc-bad">اجباری</span>':'اختیاری').'</td><td>'.self::dt($r['release_published_at']).'</td><td><div class="bvc-actions">';
                if(!empty($r['release_url']))echo '<a class="button" target="_blank" rel="noopener" href="'.esc_url($r['release_url']).'">GitHub</a>';
                if($state!=='stable'){echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_windows_release_promote_'.(int)$r['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_windows_release_promote"><input type="hidden" name="release_id" value="'.(int)$r['id'].'"><button class="button button-primary">انتشار رسمی ویندوز</button></form>';}
                if($state==='beta'){echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_windows_release_stop_'.(int)$r['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_windows_release_stop"><input type="hidden" name="release_id" value="'.(int)$r['id'].'"><button class="button">توقف نسخه آزمایشی</button></form>';}
        ...
        echo '<div class="bvc-card"><h2>سیاست به‌روزرسانی ویندوز</h2><p>نسخه رسمی و آزمایشی ویندوز مستقل از اندروید هستند. کاربر عادی نسخه رسمی را می‌گیرد؛ حسابی که در بخش کاربران Beta Tester باشد، نسخه آزمایشی فعال ویندوز را هم دریافت می‌کند.</p><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';
        ...
        self::input('windows_owner','مالک مخزن',$cfg['owner'],true);self::input('windows_repo','نام مخزن',$cfg['repo'],true);
        self::input('windows_minimum_version_stable','حداقل نسخه رسمی قابل استفاده',$cfg['minimum_version_stable'],true);self::input('windows_minimum_version_beta','حداقل نسخه آزمایشی قابل استفاده',$cfg['minimum_version_beta'],true);
        self::select('windows_site_channel','کانال نمایش در سایت',['stable'=>'رسمی','beta'=>'آزمایشی'],$cfg['site_channel']);
        ...
        echo '<label><input type="checkbox" name="windows_auto_sync" value="1" '.checked(!empty($cfg['auto_sync']),true,false).'> همگام‌سازی خودکار نسخه‌های ویندوز از مخزن</label>';
        echo '<label><input type="checkbox" name="windows_auto_update_stable" value="1" '.checked(!empty($cfg['auto_update_stable']),true,false).'> دانلود و نصب خودکار نسخه رسمی در ویندوز</label>';
        echo '<label><input type="checkbox" name="windows_auto_update_beta" value="1" '.checked(!empty($cfg['auto_update_beta']),true,false).'> 🧪 دانلود/نصب خودکار Beta برای آزمایش‌کنندگان</label>';
        echo '</div>';submit_button('ذخیره سیاست ویندوز','primary','submit',false);echo '</form></div>';
        echo '<div class="bvc-card"><h2>فرایند انتشار ویندوز</h2><div class="bvc-grid"><div><strong>① ساخت</strong><p>سامانه انتشار فایل‌های نصب ۶۴ بیتی را می‌سازد.</p></div><div><strong>② آزمایشی</strong><p>پنل نسخه جدید را خودکار آزمایشی ثبت می‌کند.</p></div><div><strong>③ رسمی</strong><p>با «انتشار رسمی ویندوز» همان فایل نصب برای همه فعال می‌شود.</p></div></div><p><strong>نشانی فنی به‌روزرسانی ویندوز:</strong></p><div class="bvc-code">'.self::esc(untrailingslashit(home_url('/')).'/wp-json/bluevpn/v1/windows/update').'</div><div class="bvc-actions" style="margin-top:12px"><form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_sync_windows_release');echo '<input type="hidden" name="action" value="bluevpn_cc_sync_windows_release">';submit_button('همگام‌سازی ویندوز همین حالا','secondary','submit',false);echo '</form><a class="button" href="'.esc_url(self::url('customers')).'">مدیریت آزمایش‌کنندگان</a></div></div>';
    }
    ...
    private static function tab_sms(): void {
        ...
        if(!empty($s['last_test_at'])){
            ...
            echo '<div class="bvc-card"><strong class="'.($ok?'bvc-ok':'bvc-bad').'">'.($ok?'✅ آخرین ارتباط SMS موفق':'❌ آخرین ارتباط SMS ناموفق').'</strong><p class="bvc-note">'.self::esc((string)($s['last_test_message']??'')).'</p><small>'.self::dt($s['last_test_at']).'</small></div>';
        }
        ...
        if($providerPatterns){
            echo '<p class="bvc-success" style="padding:10px;border:1px solid #00a32a;border-radius:8px;margin-top:12px">✅ '.number_format(count($providerPatterns)).' پترن فعال بارگذاری شده'.(!empty($patternCache['fetched_at'])?' • آخرین Sync: '.self::dt($patternCache['fetched_at']):'').'</p>';
        ...
        if($smartReport){
            ...
            echo '<div class="bvc-note"><strong>🧠 آخرین جایگذاری هوشمند:</strong> '.number_format($assigned).' تطبیق انجام شد • '.number_format($ambiguous).' مورد مبهم • '.number_format($unmatched).' بدون تطبیق امن'.(!empty($smartReport['generated_at'])?' • '.self::dt($smartReport['generated_at']):'').'.<br><small>انتخاب دستی معتبر هرگز در حالت عادی بازنویسی نمی‌شود؛ «بازچینی کامل» فقط با تأیید مدیر همه انتخاب‌ها را دوباره می‌چیند.</small></div>';
        }
        echo '</div>';
        ...
        foreach($recent as $x){echo '<tr><td>'.self::esc($x['event_key']).'</td><td>'.self::esc($x['phone']).'</td><td>'.self::esc($x['status']).'</td><td>'.self::esc($x['provider_delivery_status']?:'unknown').'</td><td><code>'.self::esc($x['provider_message_id']?:'—').'</code></td><td>'.self::esc($x['attempts']).'/'.self::esc($x['max_attempts']).'</td><td>'.self::esc(mb_substr((string)$x['last_error'],0,220)).'</td><td>'.self::dt($x['sent_at']?:$x['created_at']).'</td><td>';if(in_array($x['status'],['failed','skipped'],true)){echo '<form method="post" action="'.esc_url(admin_url('admin-post.php')).'">';wp_nonce_field('bluevpn_cc_retry_sms_'.$x['id']);echo '<input type="hidden" name="action" value="bluevpn_cc_retry_sms"><input type="hidden" name="delivery_id" value="'.esc_attr($x['id']).'"><button class="button">ارسال مجدد</button></form>';}echo '</td></tr>';}
        echo '</table></div>';
    }
    public static function save_provider(): void {
        self::guard();check_admin_referer('bluevpn_cc_save_provider');global $wpdb;
        $provider=sanitize_key((string)($_POST['provider']??''));
        $maps=[
            'pasarguard'=>'pasarguard_panels','marzban'=>'marzban_panels','guardcore'=>'guardcore_panels',
            'hiddify'=>'hiddify_panels','threexui'=>'threexui_panels'
        ];
        if(!isset($maps[$provider]))self::redirect('overview','Provider نامعتبر است.',true);
        $id=(int)($_POST['id']??0);$t=BlueVPN_DB::table($maps[$provider]);
        $old=$id?$wpdb->get_row($wpdb->prepare("SELECT * FROM {$t} WHERE id=%d",$id),ARRAY_A):[];
        $base=untrailingslashit(esc_url_raw(wp_unslash($_POST['base_url']??'')));
        if($base===''||!preg_match('~^https?://~i',$base))self::redirect($provider==='pasarguard'?'panels':$provider,'Base URL معتبر نیست.',true);
        $data=[
            'name'=>sanitize_text_field(wp_unslash($_POST['name']??'')),'base_url'=>$base,
            'verify_tls'=>isset($_POST['verify_tls'])?1:0,'active'=>isset($_POST['active'])?1:0,
        ...
        if($provider==='pasarguard'){
            $data+=['auth_mode'=>sanitize_key((string)($_POST['auth_mode']??'api_key')),'api_key_enc'=>$secret('api_key','api_key_enc'),'username_enc'=>$secret('username','username_enc'),'password_enc'=>$secret('password','password_enc'),'proxy_settings_json'=>self::sanitize_json($_POST['proxy_settings_json']??'{}','{}')];
        }elseif($provider==='marzban'){
            $data+=['username_enc'=>$secret('username','username_enc'),'password_enc'=>$secret('password','password_enc'),'proxies_json'=>self::sanitize_json($_POST['proxies_json']??'{}','{}'),'inbounds_json'=>self::sanitize_json($_POST['inbounds_json']??'{}','{}')];
        }elseif($provider==='hiddify'){
            $data+=['subscription_base_url'=>untrailingslashit(esc_url_raw(wp_unslash($_POST['subscription_base_url']??''))),'api_key_enc'=>$secret('api_key','api_key_enc'),'api_version'=>'v2'];
            if(!$id&&$data['api_key_enc']==='')self::redirect('hiddify','Hiddify API Key برای پنل جدید الزامی است.',true);
        }elseif($provider==='threexui'){
            $mode=sanitize_key((string)($_POST['auth_mode']??'api_token'));$mode=in_array($mode,['api_token','password'],true)?$mode:'api_token';
            $data+=['subscription_base_url'=>untrailingslashit(esc_url_raw(wp_unslash($_POST['subscription_base_url']??''))),'auth_mode'=>$mode,'api_token_enc'=>$secret('api_token','api_token_enc'),'username_enc'=>$secret('username','username_enc'),'password_enc'=>$secret('password','password_enc'),'api_version'=>'3.x'];
            if(!$id&&$mode==='api_token'&&$data['api_token_enc']==='')self::redirect('threexui','API Token برای حالت API Token الزامی است.',true);
            if(!$id&&$mode==='password'&&($data['username_enc']===''||$data['password_enc']===''))self::redirect('threexui','Username و Password برای حالت fallback الزامی هستند.',true);
        }else{
            $data+=['global_subscription_url'=>esc_url_raw(wp_unslash($_POST['global_subscription_url']??'')),'auth_mode'=>sanitize_key((string)($_POST['auth_mode']??'manual')),'api_key_enc'=>$secret('api_key','api_key_enc'),'username_enc'=>$secret('username','username_enc'),'password_enc'=>$secret('password','password_enc'),'usage_unit'=>sanitize_key((string)($_POST['usage_unit']??'bytes')),'expire_mode'=>sanitize_key((string)($_POST['expire_mode']??'days'))];
        }
        if($id)$ok=$wpdb->update($t,$data,['id'=>$id]);
        else{$data['created_at']=BlueVPN_Utils::now_mysql();$ok=$wpdb->insert($t,$data);$id=(int)$wpdb->insert_id;}
        if($ok!==false&&$id>0){
            $test=BlueVPN_Providers::test($provider,$id);
            $message='پنل ذخیره شد. تست اتصال: '.(string)($test['message']??'');
            self::redirect($provider==='pasarguard'?'panels':$provider,$message,empty($test['ok']));
        }
        self::redirect($provider==='pasarguard'?'panels':$provider,'ذخیره پنل ناموفق بود.',true);
    }
    ...
    public static function delete_provider(): void {
        ...
        $map=['pasarguard'=>'pasarguard_panels','marzban'=>'marzban_panels','guardcore'=>'guardcore_panels','hiddify'=>'hiddify_panels','threexui'=>'threexui_panels'];if(!isset($map[$p])||$id<=0)self::redirect('overview','Provider نامعتبر.',true);
        ...
        try{
            ...
            // Modern provider routes are canonical for all first-class providers.
            $planRows=$wpdb->get_results("SELECT id,provider_routes_json FROM {$plans} WHERE provider_routes_json IS NOT NULL AND TRIM(provider_routes_json)<>''",ARRAY_A)?:[];
            foreach($planRows as $planRow){
                $routes=BlueVPN_Utils::json_decode_array((string)$planRow['provider_routes_json'],[]);
                if(!isset($routes[$p])||!is_array($routes[$p]))continue;
                $next=array_values(array_filter($routes[$p],static fn($route)=>!is_array($route)||(int)($route['panel_id']??0)!==$id));
                if($next!==$routes[$p]){$routes[$p]=$next;if($wpdb->update($plans,['provider_routes_json'=>BlueVPN_Utils::json_encode($routes)],['id'=>(int)$planRow['id']])===false)throw new RuntimeException('پاک‌سازی Provider Routes ناموفق بود.');}
            }
            $links=BlueVPN_DB::table('customer_provider_links');
            if($wpdb->query($wpdb->prepare("DELETE FROM {$links} WHERE provider_type=%s AND panel_id=%d",$p,$id))===false)throw new RuntimeException('پاک‌سازی mappingهای Provider ناموفق بود.');
    ...
    public static function save_payment(): void { self::guard();check_admin_referer('bluevpn_cc_save_payment');global $wpdb;$t=BlueVPN_DB::table('payment_settings');$old=$wpdb->get_row("SELECT * FROM {$t} WHERE id=1",ARRAY_A)?:[];$api=trim((string)wp_unslash($_POST['api_key']??''));$base=untrailingslashit(esc_url_raw(wp_unslash($_POST['base_url']??'https://blupal.net/api')));if($base==='')$base='https://blupal.net/api';$wpdb->replace($t,['id'=>1,'provider'=>'blupal','base_url'=>$base,'api_key_enc'=>$api!==''?BlueVPN_Utils::encrypt_secret($api):(string)($old['api_key_enc']??''),'card_number'=>preg_replace('/\D+/','',(string)wp_unslash($_POST['card_number']??'')),'active'=>isset($_POST['active'])?1:0,'updated_at'=>BlueVPN_Utils::now_mysql()]);self::redirect('blupal','تنظیمات بلوپال ذخیره شد.'); }
    ...
    public static function repair_missing_provider_subscriptions(): void {
        ...
        $summary=get_transient($key);if(!is_array($summary))$summary=['total'=>BlueVPN_Providers::repairable_customer_count(),'processed'=>0,'created'=>0,'attached'=>0,'existing'=>0,'resynced'=>0,'deferred'=>0,'skipped'=>0,'error_count'=>0,'errors'=>[],'seen'=>[],'started_at'=>BlueVPN_Utils::tehran_datetime_fa()];
        ...
        if(empty($seen[(string)$id])){
            try{
                ...
                $r=['ok'=>false,'eligible'=>true,'created'=>0,'attached'=>0,'existing'=>0,'resynced'=>0,'message'=>'خطای کنترل‌شده Provider: '.$e->getMessage()];
                if(class_exists('BlueVPN_Error_Monitor'))BlueVPN_Error_Monitor::report('provider','repair','error','PROVIDER_REPAIR_UNCAUGHT',$e->getMessage(),['customer_id'=>$id]);
            }
            $seen[(string)$id]=1;$summary['seen']=$seen;$summary['processed']++;
            if(empty($r['eligible']))$summary['skipped']++;
            $summary['created']+=(int)($r['created']??0);$summary['attached']+=(int)($r['attached']??0);$summary['existing']+=(int)($r['existing']??0);$summary['resynced']+=(int)($r['resynced']??0);$summary['deferred']+=(int)($r['deferred']??0);
    ...
    public static function retry_order_provision(): void {
        self::guard();global $wpdb;$id=sanitize_text_field((string)($_POST['order_id']??''));check_admin_referer('bluevpn_cc_retry_order_provision_'.$id);$t=BlueVPN_DB::table('orders');$order=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$t} WHERE id=%s",$id),ARRAY_A);if(!$order)self::redirect('orders','سفارش پیدا نشد.',true);if(!in_array((string)$order['status'],['paid','paid_needs_sync','partial_needs_sync'],true))self::redirect('orders','این سفارش در وضعیت قابل Retry نیست.',true);
        $attempts=BlueVPN_DB::table('provisioning_attempts');$attemptNo=(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*)+1 FROM {$attempts} WHERE order_id=%s",$id));$wpdb->insert($attempts,['order_id'=>$id,'customer_id'=>(int)$order['customer_id'],'plan_id'=>(int)$order['plan_id'],'trigger_source'=>'admin_retry','attempt_no'=>$attemptNo,'status'=>'started','started_at'=>BlueVPN_Utils::now_mysql(),'created_at'=>BlueVPN_Utils::now_mysql()]);$attemptId=(int)$wpdb->insert_id;
        try{$r=BlueVPN_Providers::provision_customer((int)$order['customer_id'],(int)$order['plan_id']);$status=!empty($r['ok'])?'activated':(!empty($r['partial'])?'partial_needs_sync':'paid_needs_sync');$wpdb->update($t,['status'=>$status,'activated_at'=>$status==='activated'?BlueVPN_Utils::now_mysql():$order['activated_at'],'activation_error'=>!empty($r['ok'])?'':mb_substr((string)($r['message']??'Provision ناقص'),0,2000)],['id'=>$id]);$wpdb->update($attempts,['status'=>$status==='activated'?'success':($status==='partial_needs_sync'?'partial':'failed'),'result_json'=>BlueVPN_Utils::json_encode($r),'error_message'=>!empty($r['ok'])?'':mb_substr((string)($r['message']??''),0,2000),'finished_at'=>BlueVPN_Utils::now_mysql()],['id'=>$attemptId]);self::redirect('orders',$status==='activated'?'Provision با موفقیت کامل شد.':'Provision هنوز کامل نیست: '.($r['message']??''),$status!=='activated');}catch(Throwable $e){$wpdb->update($attempts,['status'=>'failed','error_message'=>mb_substr($e->getMessage(),0,2000),'finished_at'=>BlueVPN_Utils::now_mysql()],['id'=>$attemptId]);self::redirect('orders','تلاش دوباره برای فعال‌سازی ناموفق: '.$e->getMessage(),true);}
    }

    public static function update_customer_entitlement(): void {
        self::guard();global $wpdb;
        $customerId=max(0,(int)($_POST['customer_id']??0));
        check_admin_referer('bluevpn_cc_update_customer_entitlement_'.$customerId);
        $ct=BlueVPN_DB::table('customers');
        $before=$wpdb->get_row($wpdb->prepare("SELECT id,plan_id,phone,subscription_status,subscription_expire,data_limit_bytes FROM {$ct} WHERE id=%d LIMIT 1",$customerId),ARRAY_A);
        if(!$before)self::redirect('customers','کاربر پیدا نشد.',true);

        $expireRaw=trim(sanitize_text_field(wp_unslash((string)($_POST['absolute_expire_date']??''))));
        $targetExpiry=BlueVPN_Utils::mysql_from_tehran_date($expireRaw,true);
        if(!$targetExpiry)self::redirect('customers','تاریخ انقضای شمسی معتبر وارد کنید.',true);

        $volumeRaw=str_replace(',','.',trim(sanitize_text_field(wp_unslash((string)($_POST['absolute_data_limit_gb']??'')))));
        if($volumeRaw===''||!is_numeric($volumeRaw))self::redirect('customers','حجم نهایی باید عدد معتبر بر حسب GB باشد.',true);
        $volumeGb=(float)$volumeRaw;
        if($volumeGb<0||$volumeGb>100000)self::redirect('customers','حجم نهایی باید بین ۰ تا ۱۰۰۰۰۰ گیگابایت باشد.',true);
        $targetBytes=(int)round($volumeGb*1024*1024*1024);

        $planId=max(0,(int)($before['plan_id']??0));$result=['ok'=>true,'partial'=>false,'message'=>'اعتبار مرکزی کاربر بروزرسانی شد.'];
        if($planId>0){
            $result=BlueVPN_Providers::provision_customer($customerId,$planId,$targetExpiry,$targetBytes);
            if(empty($result['ok'])&&empty($result['partial']))self::redirect('customers','اعمال زمان/حجم روی Providerها ناموفق بود: '.(string)($result['message']??''),true);
        }else{
            $status=(strtotime($targetExpiry.' UTC')?:0)>time()?'active':'expired';
            $ok=$wpdb->update($ct,[
                'subscription_expire'=>$targetExpiry,'data_limit_bytes'=>$targetBytes,
                'subscription_status'=>$status,'last_sync_at'=>null,'last_sync_error'=>''
            ],['id'=>$customerId]);
            if($ok===false)self::redirect('customers','ذخیره زمان/حجم کاربر ناموفق بود.',true);
            $result['message']='زمان و حجم مرکزی ذخیره شد؛ این کاربر پلن Provider ندارد.';
        }

        BlueVPN_Providers::record_entitlement_ledger(
            $customerId,$planId?:null,'admin','entitlement-edit-'.wp_generate_uuid4(),'admin_entitlement_adjustment',
            false,0,(string)($before['subscription_expire']??''),$targetExpiry,
            ['actor'=>get_current_user_id(),'absolute_data_limit_bytes'=>$targetBytes,'absolute_data_limit_gb'=>$volumeGb]
        );

        if(class_exists('BlueVPN_SMS_Notifications')&&!empty($before['phone'])&&(string)($before['subscription_expire']??'')!==$targetExpiry){
            try{
                $planTitle='اشتراک';
                if($planId>0)$planTitle=(string)($wpdb->get_var($wpdb->prepare('SELECT title FROM '.BlueVPN_DB::table('plans').' WHERE id=%d LIMIT 1',$planId))?:'اشتراک');
                BlueVPN_SMS_Notifications::queue_and_dispatch(
                    'subscription_renewed',(string)$before['phone'],
                    ['plan'=>mb_substr($planTitle,0,40),'expire_date'=>BlueVPN_SMS_Notifications::jalali_date($targetExpiry)],
                    $customerId,null,'admin-entitlement-edit:'.$customerId.':'.$targetExpiry
                );
            }catch(Throwable $e){BlueVPN_Error_Monitor::legacy_error_log('BlueVPN entitlement edit SMS: '.$e->getMessage());}
        }

        $message='زمان و حجم کاربر بروزرسانی شد • انقضا: '.BlueVPN_Utils::tehran_date_fa($targetExpiry).' • حجم: '.rtrim(rtrim(number_format($volumeGb,2,'.',''),'0'),'.').' GB. '.(string)($result['message']??'');
        wp_safe_redirect(self::url('customers',['customer_id'=>$customerId,'cc_msg'=>$message]));exit;
    }
    ...
    public static function manual_activate(): void {
        ...
        $volumeRaw=trim(sanitize_text_field(wp_unslash((string)($_POST['custom_data_limit_gb']??''))));
        ...
        if($customVolume){
            $normalizedVolume=str_replace(',','.',$volumeRaw);
            if(!is_numeric($normalizedVolume))self::redirect('manual','حجم دلخواه باید عدد معتبر بر حسب GB باشد.',true);
            $volumeGb=(float)$normalizedVolume;
            if($volumeGb<0||$volumeGb>100000)self::redirect('manual','حجم دلخواه باید بین ۰ تا ۱۰۰۰۰۰ گیگابایت باشد.',true);
    ...
    public static function release_promote(): void {
        ...
        if(!empty($windowsResult['pending'])){
            self::redirect('app','Android '.$version.' رسمی شد. درخواست انتشار رسمی ویندوز نیز ثبت شد و به محض آماده‌شدن هر دو فایل نصب خودکار Stable می‌شود.',false);
        }
        self::redirect('app','نسخه '.$version.' برای اندروید و ویندوز با موفقیت Stable/رسمی شد.',false);
    }
    ...
    public static function save_windows_update_policy(): void {
        ...
        self::redirect('app','سیاست به‌روزرسانی ویندوز ذخیره شد.');
    }
    ...
    public static function sync_windows_release(): void {
        ...
        self::redirect('app',(string)($r['message']??'همگام‌سازی ویندوز انجام شد.'),empty($r['ok']));
    }
