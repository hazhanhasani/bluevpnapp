...
def main() -> None:
    ...
    # WPF FontFamily is provided by Control/TextElement; applying it directly to
    # layout containers such as Border/Grid/StackPanel fails at XAML compile time.
    invalid_font_owner = re.search(r"<(?:Border|Grid|StackPanel|DockPanel|Canvas|UniformGrid)\b[^>]*\bFontFamily\s*=", main, re.IGNORECASE | re.DOTALL)
    require(invalid_font_owner is None, "WPF FontFamily is attached to a non-Control layout container")
    require(re.search(r"<Window\b[^>]*\bFontFamily=\"Segoe UI\"", main, re.IGNORECASE | re.DOTALL) is not None, "Windows auth typography must be owned by the Window/Control scope")
    main_cs = read("bluevpn-windows/MainWindow.xaml.cs")
    windows_blueai = read("bluevpn-windows/Services/WindowsBlueAiService.cs")
    ...
    require(version == "5.1.7", "this Windows migration must be release 5.1.7")
    ...
    # 5.1.7 Windows stability gates: Android UI parity, non-blocking media/metrics,
    ...
    # 5.1.7 CI/release hardening: installers must be root-level artifacts and
