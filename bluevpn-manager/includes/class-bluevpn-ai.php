...
final class BlueVPN_AI {
    ...
    public static function render_admin(): void {
        ...
        echo '<script>(function(){const endpoint='.wp_json_encode($ajax).';const nonce='.wp_json_encode($nonce).';const interval='.(int)$refresh.';let running=false;async function refreshBlueAi(){if(running||document.visibilityState!=="visible")return;const set=(id,v)=>{const el=document.getElementById(id);if(el)el.textContent=v};running=true;try{const body=new URLSearchParams({action:"bluevpn_ai_live_snapshot",nonce:nonce});const res=await fetch(endpoint,{method:"POST",credentials:"same-origin",headers:{"Content-Type":"application/x-www-form-urlencoded;charset=UTF-8"},body:body.toString()});const json=await res.json();if(!json.success||!json.data)return;const d=json.data;set("bvai-kpi-live",d.counts.total);set("bvai-kpi-free",d.counts.free);set("bvai-kpi-premium",d.counts.premium);set("bvai-live-ping",d.ping_clients>0?d.average_live_ping_ms+" ms":"—");set("bvai-live-ping-detail",d.ping_clients>0?("min "+d.minimum_live_ping_ms+" • max "+d.maximum_live_ping_ms+" • jitter "+d.average_live_jitter_ms+" • loss "+d.average_live_loss_pct+"%"):"در انتظار نمونه واقعی");set("bvai-degraded",d.degraded_routes);set("bvai-live-updated","همین حالا");const live=document.getElementById("bluevpn-ai-live-table");if(live)live.innerHTML=d.live_html;const versions=document.getElementById("bluevpn-ai-version-table");if(versions)versions.innerHTML=d.version_html;}catch(e){set("bvai-live-updated","خطا در نوسازی");}finally{running=false;}}setInterval(refreshBlueAi,interval);document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible")refreshBlueAi();});})();</script>';
