...
final class بلووی‌پی‌ان_Admin {
    public static function init(): void {
        // Register بلووی‌پی‌ان pages before later admin_menu customizers so WordPress
    ...
    public static function menu(): void {
        add_menu_page('مرکز مدیریت بلووی‌پی‌ان','بلووی‌پی‌ان','manage_options','bluevpn-manager',[بلووی‌پی‌ان_Control_Center::class,'page'],'dashicons-shield-alt',3);
        add_submenu_page('bluevpn-manager','نمای کلی','نمای کلی','manage_options','bluevpn-manager',[بلووی‌پی‌ان_Control_Center::class,'page']);
        $sections=[
        ...
        foreach($sections as [$slug,$label,$section]){
            add_submenu_page('bluevpn-manager',$label,$label,'manage_options',$slug,static function() use($section){ بلووی‌پی‌ان_Control_Center::render_section($section); });
        }
        ...
        add_submenu_page('bluevpn-manager','به‌روزرسانی افزونه','به‌روزرسانی افزونه','manage_options','bluevpn-github-updater',[self::class,'github_updater_page']);

        // Register every page exposed by the unified بلووی‌پی‌ان sidebar here, on the
        ...
        add_submenu_page('bluevpn-manager','پشتیبانی آنلاین','پشتیبانی آنلاین','manage_options','bluevpn-support',[بلووی‌پی‌ان_Support::class,'admin_page']);
        add_submenu_page('bluevpn-manager','ربات تلگرام','ربات تلگرام','manage_options','bluevpn-telegram-bot',[بلووی‌پی‌ان_Telegram_Bot::class,'admin_page']);
        add_submenu_page('bluevpn-manager','خطاها و پایش','خطاها و پایش','manage_options','bluevpn-error-monitor',[بلووی‌پی‌ان_Error_Monitor::class,'admin_page']);

        // Do not mutate $submenu after registration. The custom بلووی‌پی‌ان sidebar is
    ...
    private static function head(string $title): void { بلووی‌پی‌ان_Unified_UI::shell_open($title); echo '<div class="wrap" dir="rtl"><style>.bvp-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px;max-width:1100px}.bvp-card{background:#fff;border:1px solid #dcdcde;border-radius:10px;padding:18px}.bvp-ok{color:#34d399;font-weight:700}.bvp-warn{color:#fbbf24;font-weight:700}.bvp-code{direction:ltr;text-align:left;background:#f6f7f7;padding:10px;border-radius:6px;overflow:auto}.bvp-table{background:#fff;max-width:1200px}.bvp-table th,.bvp-table td{text-align:right}</style>'; }
    private static function foot(): void { echo '</div>'; بلووی‌پی‌ان_Unified_UI::shell_close(); }
    public static function dashboard(): void {
        self::guard();
        $s = بلووی‌پی‌ان_DB::status();
        $counts = $s['ready'] ? بلووی‌پی‌ان_DB::counts() : [];
        $cutover = get_option('bluevpn_manager_cutover_ready','0') === '1';
        $appUrl = admin_url('admin.php?page=bluevpn-app-connection');
        self::head('مدیریت بلووی‌پی‌ان');
        echo '<div class="notice notice-success"><p><strong>✅ انتقال کامل است.</strong> وردپرس و پایگاه داده تنها مرکز کنترل فعال بلووی‌پی‌ان هستند. <a href="'.esc_url($appUrl).'">بررسی اتصال اپلیکیشن</a></p></div>';
        echo '<div class="bvp-grid">';
        echo '<div class="bvp-card"><h3>پایگاه داده</h3><p class="'.($s['ready']?'bvp-ok':'bvp-warn').'">'.($s['ready']?'آماده':'نیاز به تعمیر').'</p><small>نسخه ساختار '.esc_html($s['schema_version']).'</small></div>';
        echo '<div class="bvp-card"><h3>جداول بلووی‌پی‌ان</h3><p><strong>'.count(array_filter($counts,fn($v)=>$v>=0)).' / '.count(بلووی‌پی‌ان_DB::table_names()).'</strong></p></div>';
        echo '<div class="bvp-card"><h3>اتصال اپلیکیشن</h3><div class="bvp-code">'.esc_html(untrailingslashit(home_url('/'))).'</div><p><a class="button button-primary" href="'.esc_url($appUrl).'">تست و دریافت نشانی فنی</a></p></div>';
        echo '<div class="bvp-card"><h3>نشانی سلامت سامانه</h3><div class="bvp-code">'.esc_html(home_url('/health')).'</div></div></div>';
    ...
    public static function settings_page(): void {
        self::guard();
        $s=بلووی‌پی‌ان_DB::settings();
        self::head('تنظیمات عمومی بلووی‌پی‌ان');
        if(isset($_GET['saved'])) echo '<div class="notice notice-success"><p>تنظیمات ذخیره شد.</p></div>';
        echo '<div class="notice notice-info"><p>تنظیمات عملیاتی، پشتیبان‌گیری، اتصال وارپ، تبلیغات و انتشار از این صفحه جدا شده‌اند و در بخش تخصصی خودشان مدیریت می‌شوند.</p></div>';
        ...
        $links=[
            'سلامت و پشتیبان‌گیری'=>'bluevpn-production',
            'اتصال رایگان / اتصال وارپ'=>'bluevpn-free-access',
            'تبلیغات'=>'bluevpn-ads',
            'اپ و انتشار'=>'bluevpn-app-update',
            'خطاها و پایش'=>'bluevpn-error-monitor',
        ...
        echo '<section class="bluevpn-settings-section"><h2>نشانی فنیهای سیستم</h2><p>برای عیب‌یابی و اتصال سرویس‌ها.</p>';
    ...
    public static function app_connection_page(): void {
        ...
        self::head('اتصال اپلیکیشن بلووی‌پی‌ان');
        ...
        echo '<div class="notice notice-success"><p><strong>✅ WordPress/پایگاه داده مقصد نهایی فعال است.</strong> مسیر Legacy از Runtime خارج شده و این API مرجع Android و Windows است.</p></div>';
        echo '<div class="bvp-grid">';
        ...
        echo '</div>';
        echo '<h2>نشانی فنیهای آماده</h2><table class="widefat striped bvp-table"><tbody>';
        $eps=[
          'Health'=>$site.'/health',
        ...
        echo '</div>';
        echo '<div class="bvp-card" style="max-width:900px;margin-top:18px"><h2>Control Plane فعال</h2><pre class="bvp-endpoint">"api_base_url": "'.esc_html($site).'"</pre><p>این نشانی فنی تنها مرجع فعال بلووی‌پی‌ان است؛ Migration Bridge بازنشسته شده است.</p></div>';
        self::foot();
    }
    public static function test_app_connection(): void {
        ...
        global $wpdb;
        $assetTable=بلووی‌پی‌ان_DB::table('ad_assets');$assetId=(string)$wpdb->get_var("SELECT id FROM {$assetTable} ORDER BY created_at DESC LIMIT 1");
        if($assetId!==''){
            ...
            $out['Asset تبلیغات پایگاه داده']=['ok'=>$ok,'message'=>$ok?('تصویر قابل Decode است · '.$imageInfo[0].'×'.$imageInfo[1].' · '.strlen($body).' bytes'):(is_wp_error($r)?$r->get_error_message():'HTTP '.$code.' · '.$type.' · body='.strlen($body).' bytes')];
        } else {
            $out['Asset تبلیغات پایگاه داده']=['ok'=>true,'message'=>'هنوز تصویر محلی ثبت نشده؛ بعد از افزودن بنر این تست فعال می‌شود.'];
        }
        $aiTables=['ai_connection_events','ai_live_connections','ai_route_aggregates','ai_feedback'];$missing=[];foreach($aiTables as $name){$table=بلووی‌پی‌ان_DB::table($name);if((string)$wpdb->get_var($wpdb->prepare('SHOW TABLES LIKE %s',$table))!==$table)$missing[]=$name;}
        $out['جداول BlueAI پایگاه داده']=['ok'=>!$missing,'message'=>$missing?('جدول‌های مفقود: '.implode(', ',$missing)):'هر چهار جدول BlueAI آماده‌اند.'];
        ...
        self::app_connection_redirect($failed?'تست انجام شد؛ بعضی نشانی فنیها نیاز به بررسی دارند.':'تست کامل API، تبلیغات و BlueAI با موفقیت انجام شد.');
    }
    ...
    public static function add_plan(): void {
        ...
        $pg=max(0,(int)($_POST['panel_id']??0));$mz=max(0,(int)($_POST['marzban_panel_id']??0));$shKey=trim((string)wp_unslash($_POST['shahrah_plan_key']??''));$shId=0;$shSlug='';if($shKey!==''){[$shIdRaw,$shSlugRaw]=array_pad(explode('|',$shKey,2),2,'');$shId=max(0,(int)$shIdRaw);$shSlug=sanitize_text_field($shSlugRaw);if($shId<=0||$shSlug===''||!preg_match('/^[A-Za-z0-9._-]+$/',$shSlug)){wp_safe_redirect(add_query_arg('cc_error','پلن Shahrah انتخاب‌شده معتبر نیست.',admin_url('admin.php?page=bluevpn-plans')));exit;}}$gc=max(0,(int)($_POST['guardcore_panel_id']??0));$services=$postedIds('guardcore_service_ids_selected','guardcore_service_ids');$groups=$postedIds('group_ids_selected','group_ids');$marzbanInbounds=$mzSelected();$mode=sanitize_key((string)wp_unslash($_POST['multi_provider_quota_mode']??'split'));$mode=in_array($mode,['split','full'],true)?$mode:'split';$trafficMode=sanitize_key((string)wp_unslash($_POST['traffic_mode']??'provider_reported'));$trafficMode=$trafficMode==='gateway_metered'?'gateway_metered':'provider_reported';$gatewayReplicas=max(1,min(3,(int)($_POST['gateway_replica_count']??2)));$sourceIds=$postedIds('source_ids_selected','source_ids');
        foreach([[$pg,'pasarguard_panels'],[$mz,'marzban_panels'],[$shId,'shahrah_panels'],[$gc,'guardcore_panels']] as [$id,$table])if($id>0&&!$wpdb->get_var($wpdb->prepare('SELECT id FROM '.بلووی‌پی‌ان_DB::table($table).' WHERE id=%d',$id))){wp_safe_redirect(add_query_arg('cc_error','Provider انتخاب‌شده پیدا نشد.',admin_url('admin.php?page=bluevpn-plans')));exit;}
        if($shId>0&&(!class_exists('بلووی‌پی‌ان_Shahrah')||!بلووی‌پی‌ان_Shahrah::plan_exists($shId,$shSlug,true))){wp_safe_redirect(add_query_arg('cc_error','پلن انتخابی Shahrah در آخرین Sync این اتصال وجود ندارد؛ ابتدا Shahrah را Sync کن.',admin_url('admin.php?page=bluevpn-plans')));exit;}
        if($gc>0){$auth=$wpdb->get_var($wpdb->prepare('SELECT auth_mode FROM '.بلووی‌پی‌ان_DB::table('guardcore_panels').' WHERE id=%d',$gc));if($auth&&$auth!=='manual'&&!$services){wp_safe_redirect(add_query_arg('cc_error','برای GuardCore خودکار حداقل یک Service ID وارد کن.',admin_url('admin.php?page=bluevpn-plans')));exit;}}
        $usd=round(max(0,(float)($_POST['usd_price']??0)),6);try{$price=بلووی‌پی‌ان_Dollar_Pricing::quote_toman($usd);}catch(Throwable $e){wp_safe_redirect(add_query_arg('cc_error',$e->getMessage(),admin_url('admin.php?page=bluevpn-plans')));exit;}
        $ok=$wpdb->insert(بلووی‌پی‌ان_DB::table('plans'),['title'=>sanitize_text_field(wp_unslash($_POST['title']??'')),'description'=>sanitize_textarea_field(wp_unslash($_POST['description']??'')),'price_toman'=>$price,'usd_price'=>$usd,'usd_managed'=>1,'usd_last_price_toman'=>$price,'usd_updated_at'=>بلووی‌پی‌ان_Utils::now_mysql(),'duration_days'=>max(0,(int)($_POST['duration_days']??0)),'data_limit_gb'=>max(0,(int)($_POST['data_limit_gb']??0)),'device_limit'=>max(1,min(10,(int)($_POST['device_limit']??1))),'group_ids_json'=>بلووی‌پی‌ان_Utils::json_encode($groups),'marzban_inbounds_json'=>بلووی‌پی‌ان_Utils::json_encode($marzbanInbounds),'active'=>1,'deleted'=>0,'sort_order'=>(int)($_POST['sort_order']??0),'panel_id'=>$pg?:null,'marzban_panel_id'=>$mz?:null,'marzban_quota_mode'=>$mode,'shahrah_panel_id'=>$shId?:null,'shahrah_plan_slug'=>$shSlug,'guardcore_panel_id'=>$gc?:null,'guardcore_service_ids_json'=>بلووی‌پی‌ان_Utils::json_encode($services),'multi_provider_quota_mode'=>$mode,'traffic_mode'=>$trafficMode,'gateway_replica_count'=>$gatewayReplicas,'source_ids_json'=>بلووی‌پی‌ان_Utils::json_encode($sourceIds),'created_at'=>بلووی‌پی‌ان_Utils::now_mysql()]);
    ...
    public static function toggle_customer(): void {
        ...
        if(!empty($c['phone'])&&class_exists('بلووی‌پی‌ان_SMS_Notifications')){try{بلووی‌پی‌ان_SMS_Notifications::queue($new?'account_unblocked':'account_temporarily_blocked',(string)$c['phone'],[],$id,null,'account-status:'.$id.':'.$new.':'.gmdate('YmdHi'));}catch(Throwable $e){بلووی‌پی‌ان_Error_Monitor::legacy_error_log('بلووی‌پی‌ان account status SMS: '.$e->getMessage());}}
        wp_safe_redirect(admin_url('admin.php?page=bluevpn-customers'));exit;
    }
    public static function migration_page(): void {
        self::guard();
        if (class_exists('بلووی‌پی‌ان_Production')) بلووی‌پی‌ان_Production::ensure_native_control_plane();
        $db=بلووی‌پی‌ان_DB::status();$counts=بلووی‌پی‌ان_DB::counts();
        $finalized=(string)get_option('bluevpn_manager_production_finalized_at','');
        self::head('وضعیت انتقال بلووی‌پی‌ان');
        echo '<div class="notice notice-success"><p><strong>✅ مهاجرت نهایی شده است.</strong> بک‌اند Legacy از Runtime حذف شده و وردپرس و پایگاه داده تنها مرکز کنترل فعال بلووی‌پی‌ان هستند.</p></div>';
        echo '<div class="bvp-grid">';
        echo '<div class="bvp-card"><h3>Control Plane</h3><p class="bvp-ok">WordPress / پایگاه داده</p><small>حالت Native و دائمی</small></div>';
        ...
        echo '<div class="bvp-card"><h3>نهایی‌سازی</h3><p>'.esc_html($finalized!==''?بلووی‌پی‌ان_Utils::tehran_datetime_fa($finalized):'در حال ثبت').'</p></div>';
        echo '</div>';
        echo '<h2>نشانی فنی فعال</h2><div class="bvp-code">'.esc_html(untrailingslashit(home_url('/'))).'</div>';
    ...
    public static function migration_generate_token(): void {
        ...
        $cfg = بلووی‌پی‌ان_Migration::settings();
        بلووی‌پی‌ان_Migration::save_settings([
    ...
    public static function migration_test(): void {
        self::guard(); check_admin_referer('bluevpn_migration_test'); $r=بلووی‌پی‌ان_Migration::test_connection();
    ...
    public static function migration_manifest(): void {
        self::guard(); check_admin_referer('bluevpn_migration_manifest'); $r=بلووی‌پی‌ان_Migration::refresh_manifest();
        if (is_wp_error($r)) self::migration_redirect('', $r->get_error_message());
        self::migration_redirect('Manifest دریافت شد و تعداد جدول‌ها به‌روزرسانی شد.');
    }
    public static function migration_run(): void {
        self::guard(); check_admin_referer('bluevpn_migration_run'); بلووی‌پی‌ان_Migration::start_auto(); $r=بلووی‌پی‌ان_Migration::run(12);
    ...
    public static function migration_resync(): void {
        ...
        $manifest = بلووی‌پی‌ان_Migration::refresh_manifest();
        if (is_wp_error($manifest)) self::migration_redirect('', $manifest->get_error_message());
        $status = بلووی‌پی‌ان_Migration::dashboard_status(false);
        ...
        if (!$targets) {
            بلووی‌پی‌ان_Migration::start_auto();
            بلووی‌پی‌ان_Migration::auto_step(4);
        ...
        foreach ($targets as $target) {
            ...
            if ($gap > 0 && $gap <= 500 && $sourceCount <= 100000) {
                $exact = بلووی‌پی‌ان_Migration::repair_exact_missing($target, min(500, max(20, $gap + 20)));
        ...
        $manifest = بلووی‌پی‌ان_Migration::refresh_manifest();
        if (is_wp_error($manifest)) self::migration_redirect('', $manifest->get_error_message());
        $after = بلووی‌پی‌ان_Migration::dashboard_status(false);
        ...
        if (!$still) {
            بلووی‌پی‌ان_Migration::start_auto();
            بلووی‌پی‌ان_Migration::auto_step(2);
            self::migration_redirect('ترمیم دقیق موفق بود؛ '.$exactImported.' رکورد گمشده بازیابی شد و Verify نهایی ادامه دارد.');
        }

        بلووی‌پی‌ان_Migration::start_resync(false, $still, 'delta');
        بلووی‌پی‌ان_Migration::start_auto();
        بلووی‌پی‌ان_Migration::run(8);
    ...
    public static function migration_auto_start(): void {
        ...
        $before = بلووی‌پی‌ان_Migration::dashboard_status(false);
        if (!empty($before['readiness']['ready'])) {
            $manifest = بلووی‌پی‌ان_Migration::refresh_manifest();
            if (is_wp_error($manifest)) self::migration_redirect('', $manifest->get_error_message());
            $fresh = بلووی‌پی‌ان_Migration::dashboard_status(true);
        ...
        بلووی‌پی‌ان_Migration::start_auto();
        بلووی‌پی‌ان_Migration::auto_step(4);
        $status = بلووی‌پی‌ان_Migration::dashboard_status(true);
    ...
    public static function migration_pump(): void {
        ...
        بلووی‌پی‌ان_Migration::auto_step(3);
        $status = بلووی‌پی‌ان_Migration::dashboard_status(true);
    ...
    public static function migration_auto_stop(): void {
        ...
        بلووی‌پی‌ان_Migration::stop_auto();
        self::migration_redirect('انتقال خودکار متوقف شد.');
    }

    public static function migration_reset(): void {
        self::guard(); check_admin_referer('bluevpn_migration_reset'); بلووی‌پی‌ان_Migration::reset(true); بلووی‌پی‌ان_Migration::mark_cutover_ready(false); self::migration_redirect('Progress مهاجرت ریست شد؛ داده‌های پایگاه داده حذف نشدند.');
    }
    public static function migration_cutover(): void {
        ...
        if ($ready && !بلووی‌پی‌ان_Migration::mark_cutover_ready(true)) {
            self::migration_redirect('', 'Cutover فقط بعد از Resync نهایی و تطبیق واقعی Railway/پایگاه داده فعال می‌شود.');
        }
        بلووی‌پی‌ان_Migration::mark_cutover_ready($ready);
    ...
    public static function github_updater_page(): void {
        self::guard();
        $cfg = بلووی‌پی‌ان_GitHub_Updater::settings();
        $release = بلووی‌پی‌ان_GitHub_Updater::latest_release(false);
        self::head('آپدیت مدیریت بلووی‌پی‌ان از GitHub');
        ...
        echo '<div class="bvp-card"><h3>مخزن</h3><p><a href="'.esc_url(بلووی‌پی‌ان_GitHub_Updater::repository_url()).'" target="_blank" rel="noopener">'.esc_html($cfg['owner'].'/'.$cfg['repo']).'</a></p></div>';
        if (is_wp_error($release)) {
            ...
            $repair = بلووی‌پی‌ان_GitHub_Updater::update_available($release) && version_compare($remote, BLUEVPN_MANAGER_VERSION, '==');
        ...
        $diag = بلووی‌پی‌ان_GitHub_Updater::diagnostics();
        ...
        $last_bg = بلووی‌پی‌ان_GitHub_Updater::last_background_check();
        echo '<div class="bvp-card"><h3>بررسی خودکار GitHub</h3><p class="bvp-ok"><strong>هر ۲ دقیقه</strong></p><small>'.($last_bg ? 'آخرین اجرا: '.esc_html(بلووی‌پی‌ان_Utils::tehran_datetime_fa((int)$last_bg)) : 'در انتظار اولین اجرای پس‌زمینه').'</small></div>';
        $auto_status = بلووی‌پی‌ان_GitHub_Updater::auto_update_status();
        ...
        echo '<div class="bvp-card"><h3>آخرین نصب خودکار</h3><p class="'.esc_attr($auto_class).'"><strong>'.esc_html((string)$auto_status['status']).'</strong></p><small>'.esc_html((string)$auto_status['message']).($auto_status['at'] ? ' · '.esc_html(بلووی‌پی‌ان_Utils::tehran_datetime_fa((int)$auto_status['at'])) : '').'</small></div>';
        echo '</div>';
        ...
        echo '<tr><th>آپدیت خودکار</th><td><label><input type="checkbox" name="auto_update" value="1" '.checked(!empty($cfg['auto_update']),true,false).'> مدیریت بلووی‌پی‌ان نسخه جدید را خودش پیدا کند و بدون ورود مدیر همان لحظه نصب کند.</label></td></tr>';
        echo '</table>';
    ...
    public static function save_github_updater(): void {
        ...
        بلووی‌پی‌ان_GitHub_Updater::save_settings([
    ...
    public static function check_github_update(): void {
        ...
        بلووی‌پی‌ان_GitHub_Updater::clear_cache();
        $release = بلووی‌پی‌ان_GitHub_Updater::latest_release(true);
        delete_site_transient('update_plugins');
        ...
        if (is_wp_error($release)) {
            ...
            $message = 'هنوز Release مخصوص مدیریت بلووی‌پی‌ان در GitHub پیدا نشد.';
        } else {
            $remote = (string)($release['base_version'] ?? $release['version'] ?? '0.0.0');
            if (version_compare($remote, BLUEVPN_MANAGER_VERSION, '>')) {
            ...
            } elseif (بلووی‌پی‌ان_GitHub_Updater::update_available($release)) {
                $message = 'بسته اصلاحی جدید برای نسخه '.$remote.' موجود است.';
            } else {
                $message = 'مدیریت بلووی‌پی‌ان به‌روز است ('.$remote.').';
            }
        }
        wp_safe_redirect(add_query_arg(['page'=>'bluevpn-github-updater','checked'=>$message], admin_url('admin.php')));
        exit;
    }
    public static function repair(): void { self::guard();check_admin_referer('bluevpn_repair');بلووی‌پی‌ان_DB::install_schema();بلووی‌پی‌ان_DB::seed_defaults();بلووی‌پی‌ان_DB::seed_release_channels();بلووی‌پی‌ان_Compat::register_rewrites();flush_rewrite_rules(false);wp_safe_redirect(admin_url('admin.php?page=bluevpn-migration&repaired=1'));exit; }
}
