...
final class BlueVPN_Providers {
        ...
        try{
            ...
            if($provider==='pasarguard'){
                ...
                if(($p['auth_mode']??'manual')==='manual'){$ok=true;$msg='GuardCore در حالت دستی فعال است.';}
                    ...
                    $headers=[];$key=BlueVPN_Utils::decrypt_secret((string)($p['api_key_enc']??''));if($key!=='')$headers['X-API-Key']=$key;
                    $r=self::req('GET',self::join_url($base,'/health'),$headers,null,$ssl);$ok=$r['code']<400;$msg=$ok?'GuardCore پاسخ داد.':'HTTP '.$r['code'].' '.mb_substr($r['body'],0,220);
                }
            }
    ...
    public static function provision_customer(int $customerId,int $planId): array {
        ...
        if(!empty($plan['guardcore_panel_id'])){
            $p=self::panel('guardcore',(int)$plan['guardcore_panel_id']);if($p&&($p['auth_mode']??'manual')==='manual'){$update['guardcore_panel_id']=$p['id'];$update['guardcore_username']=self::username($c,'gc');$update['guardcore_status']=!empty($c['guardcore_subscription_url'])?'active':'manual_pending';$update['guardcore_expire']=$expire;$update['guardcore_data_limit_bytes']=$quota;$success++;}elseif($p){$errors[]='GuardCore API: Provision خودکار این Adapter هنوز فعال نشده؛ پنل را فعلاً Manual نگه دارید.';}else{$errors[]='GuardCore: پنل پیدا نشد.';}
        }
    ...
    public static function sync_customer(int $customerId): array {
        global $wpdb;$ct=BlueVPN_DB::table('customers');$c=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$ct} WHERE id=%d",$customerId),ARRAY_A);if(!$c)return ['ok'=>false,'message'=>'کاربر پیدا نشد.'];$u=[];$errors=[];$active=false;$used=0;$limit=(int)$c['data_limit_bytes'];$exp=[];
