...
## 4.11.7 WARP failure diagnostics
...
## 4.11.7 — BlueAI Control Plane v3
...
## 4.11.7 — Production Runtime Validation
...
## 4.11.7 — Native Network Adaptation
...
## 4.11.7 — Premium Instant UI + Live Support Phase 2
...
## 4.11.7 — Deploy Bot Commit/Version Verification
...
## 4.11.7 — Deploy ZIP Root Detection
...
## 4.11.7 — Support EditText Kotlin Compile Fix
...
## 4.11.7 — Background Reliability + GuardCore Visibility
...
most-usage, reached subscriptions, plan-level service selection, per-subscription
online/usage details, official enable/disable/revoke/reset actions, node
enable/disable controls, and expiry reconciliation.

The legacy configurable `limit_usage` and `limit_expire` unit modes remain because
the supplied schema identifies these as integers but does not define their units.
BlueVPN therefore does not invent or hard-code undocumented unit semantics.


## 4.11.7 — Connection Continuity
...
CoreVpnService must be down, or for Free/WARP the WARP engine must also be down.
Activity/process recovery preserves live transports. The Free/WARP notification
updater is also actually scheduled, fixing stale speed updates on that path.


## 4.11.7 — Background Full-Pool AI Optimization
...
for six hours, and classified as Fast, Stable, Reserve, or Failed. Two fresh
v2rayNG TestService passes are used per route to derive average RTT, jitter,
repeatability/loss, and a quality score. SmartSelector now consumes this
background evidence and the Servers UI exposes the category discovered on the
user's network.


## 4.11.7 — Panel payment/site/free-pool upgrade
...
## 4.11.7 — Iran-local Panel Date/Time
...
## 4.11.7 — WARP-Off Smart Free Pool
Disabling WARP no longer disables the Free entitlement. The curated Telegram/public
pool is exposed as a first-class `smart-curated` subscription, imported proactively
on pool-only Android, then passed to the background full-pool optimizer for
network-specific testing/ranking before connection.


## 4.11.8 — Support + GuardCore Release Hardening
Live Support now has explicit Department → Topic → Message routing, retry-safe
idempotent conversation/message submission, validated operator routing, support
topic management, priority-aware queues, recreation-safe drafts, and a joined
conversation query that removes N+1 metadata lookups.

The Provider repair scan now covers GuardCore API subscriptions alongside
PasarGuard and Marzban. Lost GuardCore mappings are recovered with strict
identity checks and plan Service IDs are synchronized without modifying quota,
usage, or entitlement expiry.
