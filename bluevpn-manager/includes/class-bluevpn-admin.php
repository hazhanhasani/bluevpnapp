...
final class BlueVPN_Admin {
    ...
    public static function add_plan(): void {
        ...
        foreach([[$pg,'pasarguard_panels'],[$mz,'marzban_panels'],[$shId,'shahrah_panels'],[$gc,'guardcore_panels']] as [$id,$table])if($id>0&&!$wpdb->get_var($wpdb->prepare('SELECT id FROM '.BlueVPN_DB::table($table).' WHERE id=%d',$id))){wp_safe_redirect(add_query_arg('cc_error','Provider انتخاب‌شده پیدا نشد.',admin_url('admin.php?page=bluevpn-plans')));exit;}
        if($shId>0&&(!class_exists('BlueVPN_Shahrah')||!BlueVPN_Shahrah::plan_exists($shId,$shSlug,true))){wp_safe_redirect(add_query_arg('cc_error','پلن انتخابی Shahrah در آخرین Sync این اتصال وجود ندارد؛ ابتدا Shahrah را Sync کن.',admin_url('admin.php?page=bluevpn-plans')));exit;}
