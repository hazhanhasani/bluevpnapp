...
final class BlueVPN_Control_Center {
    ...
    private static function tab_overview(): void {
        global $wpdb;$stats=[
            ['کاربران',self::count('customers')],['مشتریان دستی',self::count('manual_customers')],['اشتراک فعال',self::count('customers',"active=1 AND subscription_status='active'")],['پرداخت موفق',self::count('orders',"status IN ('paid','activated','paid_needs_sync','partial_needs_sync')")],['پلن فعال',self::count('plans','active=1 AND deleted=0')],['تأمین‌کننده فعال',self::count('pasarguard_panels','active=1')+self::count('marzban_panels','active=1')+self::count('shahrah_panels','active=1')+self::count('guardcore_panels','active=1')],['اتصال زنده',self::count('ai_live_connections','connected=1 AND verified=1')]
        ];
    ...
    private static function tab_sources(): void { BlueVPN_Subscription_منبعs::render_admin_tab(); }
    private static function tab_gateway(): void { BlueVPN_دروازه::render_admin_tab(); }
    ...
    private static function tab_production(): void {
        ...
        echo '<div class="bvc-card"><h3>پشتیبان‌گیری خودکار</h3><p class="'.(!empty($backup['ok'])?'bvc-ok':'bvc-warn').'">'.(!empty($backup['ok'])?'فعال و سالم':'هنوز پشتیبان‌گیری سالم ثبت نشده').'</p><small>'.self::dt($backup['created_at']??'').'</small></div>';
        ...
        $labels=['database'=>'دیتابیس','sms_queue'=>'صف SMS','payments'=>'پرداخت‌ها','backup'=>'پشتیبان‌گیری','cron'=>'Cron','cutover'=>'Cutover اپ','providers'=>'تأمین‌کنندهها'];
        ...
        echo '</table></div>';
        echo '<div class="bvc-grid"><div class="bvc-card"><h2>پشتیبان‌گیری خصوصی</h2><p>روزانه یک Snapshot از تمام جدول‌های BlueVPN ساخته می‌شود و ۷ نسخه اخیر نگه داشته می‌شود. قبل از هر Restore هم یک پشتیبان‌گیری خودکار گرفته می‌شود.</p>';
    ...
    private static function guardcore_api_dashboard(): void {
        ...
        foreach($panels as $panel){
            ...
            $catalog=BlueVPN_تأمین‌کنندهs::guardcore_catalog((int)$panel['id'],false);
            ...
            $reached=BlueVPN_تأمین‌کنندهs::guardcore_reached((int)$panel['id'],10);
            if($reached){
    ...
    private static function guardcore_assignments_panel(): void {
        ...
        foreach($rows as $row){
            ...
            $stats=BlueVPN_تأمین‌کنندهs::subscription_snapshot_stats((int)$row['id']);
            $row['_snapshot']=$stats;
        ...
        if($inspectPanel>0&&$inspectUser!==''){
            $detail=BlueVPN_تأمین‌کنندهs::guardcore_subscription_detail($inspectPanel,$inspectUser);
    ...
    public static function provider_access_catalog(): void {
        ...
        try{$catalog=BlueVPN_تأمین‌کنندهs::access_catalog($provider,$id,12);wp_send_json_success($catalog);}catch(Throwable $e){wp_send_json_error(['message'=>mb_substr($e->getMessage(),0,500)],502);}
    }

    private static function tab_plans(): void {
        global $wpdb;
        $p=BlueVPN_DB::table('plans');$pg=BlueVPN_DB::table('pasarguard_panels');$mz=BlueVPN_DB::table('marzban_panels');$sh=BlueVPN_DB::table('shahrah_panels');$gc=BlueVPN_DB::table('guardcore_panels');
        ...
        $select=function(string $name,array $items,$current=0,string $none='بدون تأمین‌کننده'){
        ...
        echo '<div><h2 class="bvc-section-title">مدیریت پلن‌ها</h2><p class="bvc-section-subtitle">پلن، قیمت و مسیر تأمین‌کنندهها را بدون جدول شلوغ مدیریت کن.</p></div>';
        ...
        echo '<summary><span><strong>افزودن پلن جدید</strong><small>مشخصات فروش، محدودیت‌ها و مسیر تأمین‌کننده</small></span><span class="bvc-summary-chevron">⌄</span></summary>';
        ...
        echo '<div class="bvc-form-section"><div class="bvc-form-section-head"><strong>مسیر سرویس</strong><small>تأمین‌کنندههای مورد استفاده برای این پلن</small></div><div class="bvc-form-grid">';
        ...
        self::select('multi_provider_quota_mode','نحوه اعمال حجم',['split'=>'تقسیم حجم بین تأمین‌کنندهها','full'=>'حجم کامل روی هر تأمین‌کننده'],'split');
        self::select('traffic_mode','مرجع محاسبه حجم',['provider_reported'=>'تأمین‌کننده reported (قدیمی)','gateway_metered'=>'BlueVPN دروازه (مرکزی)'],'gateway_metered');self::input('gateway_replica_count','تعداد دروازه همزمان (1 تا 3)','2',false,'number');
        echo '</div>';self::access_picker('pasarguard','panel_id',[]);self::access_picker('marzban','marzban_panel_id',[]);self::guardcore_service_picker('guardcore_panel_id',$gcRows,[]);BlueVPN_Subscription_منبعs::render_plan_picker([]);echo '</div>';

        echo '<details class="bvc-advanced-options"><summary>تنظیمات پیشرفته تأمین‌کننده</summary><div class="bvc-helper">GuardCore Serviceها از API رسمی پنل خوانده می‌شوند؛ ID دستی دیگر لازم نیست.</div></details>';
        ...
        foreach($rows as $x){
            ...
            $sourceSelected=BlueVPN_Subscription_منبعs::source_ids_for_plan($x);
            ...
            echo '<span class="bvc-provider-pill '.($trafficMode==='gateway_metered'?'is-on':'is-neutral').'">Traffic: '.($trafficMode==='gateway_metered'?'دروازه':'تأمین‌کننده').'</span>';if($trafficMode==='gateway_metered')echo '<span class="bvc-provider-pill is-on">HA ×'.$gatewayReplicas.'</span>';
            if($sourceSelected)echo '<span class="bvc-provider-pill is-on">Manual منبعs: '.count($sourceSelected).'</span>';
            echo '</div>';

            echo '<details class="bvc-plan-routing">';
            echo '<summary><span>تنظیم مسیر و تأمین‌کننده</span><span class="bvc-summary-chevron">⌄</span></summary>';
            ...
            echo '<label>نحوه اعمال حجم<select name="multi_provider_quota_mode"><option value="split" '.selected($quotaMode,'split',false).'>تقسیم بین تأمین‌کنندهها</option><option value="full" '.selected($quotaMode,'full',false).'>حجم کامل روی هر تأمین‌کننده</option></select></label>';
            echo '<label>مرجع حجم<select name="traffic_mode"><option value="provider_reported" '.selected($trafficMode,'provider_reported',false).'>تأمین‌کننده reported</option><option value="gateway_metered" '.selected($trafficMode,'gateway_metered',false).'>BlueVPN دروازه</option></select></label>';echo '<label>دروازه replicas<input type="number" name="gateway_replica_count" min="1" max="3" value="'.$gatewayReplicas.'"></label>';
            echo '</div>';self::access_picker('pasarguard','panel_id',$groupSelected);self::access_picker('marzban','marzban_panel_id',$mzSelected);BlueVPN_Subscription_منبعs::render_plan_picker($sourceSelected);
            ...
            echo '<label>مرجع حجم<select name="traffic_mode"><option value="provider_reported" '.selected($trafficMode,'provider_reported',false).'>تأمین‌کننده reported</option><option value="gateway_metered" '.selected($trafficMode,'gateway_metered',false).'>BlueVPN دروازه</option></select></label>';echo '<label>دروازه replicas<input type="number" name="gateway_replica_count" min="1" max="3" value="'.$gatewayReplicas.'"></label>';
            echo '</div>';self::guardcore_service_picker('guardcore_panel_id',$gcRows,BlueVPN_Utils::json_decode_array((string)($x['guardcore_service_ids_json']??''),[]));BlueVPN_Subscription_منبعs::render_plan_picker($sourceSelected);echo '<div class="bvc-form-grid">';
    ...
    private static function tab_customers(): void {
        ...
        $repairable=BlueVPN_تأمین‌کنندهs::repairable_customer_count();$lastRepair=get_option('bluevpn_provider_repair_last_result',[]);if(!is_array($lastRepair))$lastRepair=[];$repairNonce=wp_create_nonce('bluevpn_cc_repair_missing_provider_subscriptions');
        echo '<div class="bvc-card" id="bvc-provider-repair-card"><div style="display:flex;justify-content:space-between;gap:14px;align-items:flex-start;flex-wrap:wrap"><div><h2 style="margin-bottom:6px">🧩 همگام‌سازی اشتراک‌های گمشده تأمین‌کننده</h2><div class="bvc-note">کاربران دارای اشتراک فعال را روی همه مسیرهای پولی بررسی می‌کند: PasarGuard، Marzban و GuardCore، همچنین Shahrah، منبعهای ثابت و دروازه Metered. اشتراک گمشده با اعتبار فعلی بازیابی می‌شود، دسترسی Group/Inbound/Service طبق پلن دوباره Sync می‌شود، Shahrah در Repair هرگز Renew نمی‌شود و فقط Service موجود را پیدا/Attach یا در صورت نبود واقعی Create می‌کند؛ منبعهای ثابت Snapshot را بازسازی می‌کنند و دروازه sessionهای گمشده را دوباره جایگذاری می‌کند. این عملیات تاریخ اشتراک را تمدید نمی‌کند؛ تاریخ اشتراک WordPress بدون تغییر می‌ماند و مصرف را Reset نمی‌کند.</div></div><div class="bvc-kpi"><span>کاربر قابل اسکن</span><strong>'.number_format($repairable).'</strong></div></div>';
    ...
    private static function tab_orders(): void {
        ...
        echo '<div class="bvc-card"><h2>مدیریت پرداخت و فعال‌سازی</h2><p>پرداخت و Provision دو مرحله مستقل ثبت می‌شوند. اگر پرداخت تأیید شده ولی تأمین‌کننده ناقص باشد، بدون ساخت فاکتور جدید می‌توان Provision را Retry کرد.</p></div>';
    ...
    private static function tab_android(): void {
        ...
        if(!$releases){echo '<p class="bvc-warn">هنوز نسخه‌ای ثبت نشده است. اگر نسخه رسمی فعلی در تنظیمات قبلی وجود داشته باشد، با ارتقای Schema به‌عنوان نسخه رسمی اولیه می‌شود.</p>';}
            ...
            foreach($releases as $r){
                ...
                if(!empty($r['release_url']))echo '<a class="button" target="_blank" rel="noopener" href="'.esc_url($r['release_url']).'">مشاهده مخزن</a>';
                if($state!=='stable'){
        ...
        echo '<div class="bvc-card"><h2>اتوماسیون انتشار</h2><div class="bvc-grid"><div><strong>① ساخت</strong><p>سامانه انتشار فایل نصب اندروید را می‌سازد.</p></div><div><strong>② آزمایشی</strong><p>سامانه مدیریت نسخه جدید را خودکار آزمایشی ثبت می‌کند؛ فقط آزمایش‌کنندگان می‌بینند.</p></div><div><strong>③ رسمی</strong><p>دکمه اصلی «انتشار رسمی اندروید و ویندوز» نسخه هم‌نام را روی هر دو پلتفرم رسمی می‌کند؛ گزینه «فقط اندروید» برای انتشار مستقل باقی مانده است.</p></div></div>';
        ...
        echo '<div class="bvc-note"><strong>نکته راه‌اندازی اولیه:</strong> نسخه‌های قبل از 4.3.1 هنگام درخواست تنظیمات، توکن حساب را ارسال نمی‌کنند. بنابراین اولین آزمایش‌کنندگان باید فایل نصب اندروید نسخه 4.3.1 را یک‌بار دستی نصب کنند؛ از 4.3.1 به بعد دریافت نسخه آزمایشی کاملاً خودکار و بر اساس حساب پنل است.</div>';
    }
    ...
    private static function render_windows_release_management(int $betaCount): void {
        ...
        echo '<div class="bvc-grid">';
        echo '<div class="bvc-card bvc-kpi"><span>نسخه رسمی ویندوز</span><strong>'.self::esc($stable['version']??'—').'</strong><small>'.($stable?'برای همه کاربران ویندوز':'هنوز نسخه رسمی ویندوز ندارد').'</small></div>';
        ...
        if(!$releases){echo '<p class="bvc-warn">هنوز فایل نصب ویندوز در پنل ثبت نشده است. «همگام‌سازی ویندوز» را بزن؛ نسخه‌های ویندوز از مخزن دریافت و نسخه‌های جدید به‌صورت آزمایشی ثبت می‌شوند.</p>';}
        else{
            ...
            foreach($releases as $r){
                ...
                echo '<tr><td><strong>'.self::esc($r['version']).'</strong><br><small>کد '.(int)$r['version_code'].'</small></td><td>'.self::esc($label).'</td><td><small>۶۴ بیتی '.(!empty($r['installer_x64_url'])?'✅':'❌').' '.(!empty($x64['sha256'])?'اثر انگشت ✅':'اثر انگشت ⚠️').'<br>آرم ۶۴ بیتی '.(!empty($r['installer_arm64_url'])?'✅':'❌').' '.(!empty($arm['sha256'])?'اثر انگشت ✅':'اثر انگشت ⚠️').'</small></td><td>'.(!empty($r['force_update'])?'<span class="bvc-bad">اجباری</span>':'اختیاری').'</td><td>'.self::dt($r['release_published_at']).'</td><td><div class="bvc-actions">';
                if(!empty($r['release_url']))echo '<a class="button" target="_blank" rel="noopener" href="'.esc_url($r['release_url']).'">مشاهده مخزن</a>';
    ...
    public static function toggle_provider(): void { self::guard();global $wpdb;$p=sanitize_key((string)($_GET['provider']??''));$id=(int)($_GET['id']??0);check_admin_referer('bluevpn_cc_toggle_provider_'.$p.'_'.$id);$map=['pasarguard'=>'pasarguard_panels','marzban'=>'marzban_panels','guardcore'=>'guardcore_panels'];if(!isset($map[$p]))self::redirect('overview','تأمین‌کننده نامعتبر.',true);$t=BlueVPN_DB::table($map[$p]);$v=(int)$wpdb->get_var($wpdb->prepare("SELECT active FROM {$t} WHERE id=%d",$id));$wpdb->update($t,['active'=>$v?0:1],['id'=>$id]);self::redirect($p==='pasarguard'?'panels':$p,'وضعیت تأمین‌کننده تغییر کرد.'); }
    public static function delete_provider(): void {
        ...
        $map=['pasarguard'=>'pasarguard_panels','marzban'=>'marzban_panels','guardcore'=>'guardcore_panels'];if(!isset($map[$p])||$id<=0)self::redirect('overview','تأمین‌کننده نامعتبر.',true);
    ...
    public static function test_provider(): void { self::guard();$p=sanitize_key((string)($_GET['provider']??''));$id=(int)($_GET['id']??0);check_admin_referer('bluevpn_cc_test_provider_'.$p.'_'.$id);$r=BlueVPN_تأمین‌کنندهs::test($p,$id);self::redirect($p==='pasarguard'?'panels':$p,$r['message'],!$r['ok']); }
    ...
    public static function create_private_backup(): void {
        self::guard();check_admin_referer('bluevpn_cc_create_private_backup');try{$r=BlueVPN_Production::create_backup('manual-admin');self::redirect('production','پشتیبان‌گیری خصوصی ساخته شد: '.(string)$r['filename']);}catch(Throwable $e){self::redirect('production','پشتیبان‌گیری ناموفق: '.$e->getMessage(),true);}
    }
    ...
    public static function restore_backup(): void {
        ...
        if(empty($_FILES['backup_file'])||!is_array($_FILES['backup_file'])||(int)($_FILES['backup_file']['error']??UPLOAD_ERR_NO_FILE)!==UPLOAD_ERR_OK)self::redirect('production','فایل پشتیبان‌گیری دریافت نشد.',true);
        $size=(int)($_FILES['backup_file']['size']??0);if($size<=0||$size>100*1024*1024)self::redirect('production','حجم فایل پشتیبان‌گیری معتبر نیست.',true);
        $tmp=(string)($_FILES['backup_file']['tmp_name']??'');$json=@file_get_contents($tmp);if(!is_string($json))self::redirect('production','خواندن فایل پشتیبان‌گیری ناموفق بود.',true);
        try{$r=BlueVPN_Production::restore_from_json($json);self::redirect('production','Restore با موفقیت انجام شد؛ نسخه مبدا '.(string)$r['source_version'].'، پشتیبان‌گیری قبل از Restore: '.(string)$r['pre_restore_backup']);}catch(Throwable $e){self::redirect('production','Restore ناموفق و Transaction برگشت داده شد: '.$e->getMessage(),true);}
    }

    public static function finalize_cutover(): void {
        self::guard();check_admin_referer('bluevpn_cc_finalize_cutover');if(empty($_POST['confirmed']))self::redirect('production','تأیید نهایی لازم است.',true);try{$r=BlueVPN_Production::finalize_cutover();self::redirect('production','Production Cutover نهایی شد؛ Migration Token پاک شد. پشتیبان‌گیری: '.(string)$r['backup']);}catch(Throwable $e){self::redirect('production','نهایی‌سازی انجام نشد: '.$e->getMessage(),true);}
    }
    ...
    public static function repair_customer_providers(): void {
        ...
        $r=BlueVPN_تأمین‌کنندهs::repair_customer_missing_providers($id);
        self::redirect('customers',(string)($r['message']??'ترمیم تأمین‌کننده انجام شد.'),empty($r['ok']));
    }
    ...
    public static function repair_missing_provider_subscriptions(): void {
        ...
        $summary=get_transient($key);if(!is_array($summary))$summary=['total'=>BlueVPN_تأمین‌کنندهs::repairable_customer_count(),'processed'=>0,'created'=>0,'attached'=>0,'existing'=>0,'resynced'=>0,'skipped'=>0,'error_count'=>0,'errors'=>[],'seen'=>[],'started_at'=>BlueVPN_Utils::tehran_datetime_fa()];
        $ids=BlueVPN_تأمین‌کنندهs::repair_candidate_ids_after($cursor,1);
        if(!$ids){
        ...
        if(empty($seen[(string)$id])){
            $r=BlueVPN_تأمین‌کنندهs::repair_customer_missing_providers($id);$seen[(string)$id]=1;$summary['seen']=$seen;$summary['processed']++;
        ...
        $next=BlueVPN_تأمین‌کنندهs::repair_candidate_ids_after($id,1);$done=empty($next);
    ...
    public static function sync_customer(): void { self::guard();$id=(int)($_GET['customer_id']??0);check_admin_referer('bluevpn_cc_sync_customer_'.$id);$r=BlueVPN_تأمین‌کنندهs::sync_customer($id,true);self::redirect('customers',$r['message'],!$r['ok']); }
    ...
    public static function manual_activate(): void {
        ...
        $targetExpiry=BlueVPN_تأمین‌کنندهs::next_entitlement_expiry($customerId,$planId);
        $manualRef='manual-'.wp_generate_uuid4();
        BlueVPN_تأمین‌کنندهs::record_entitlement_ledger($customerId,$planId,'admin',$manualRef,'intentional_grant',true,max(0,(int)($plan['duration_days']??0)),$before['subscription_expire']??null,$targetExpiry,['actor'=>get_current_user_id()]);
        $r=BlueVPN_تأمین‌کنندهs::provision_customer($customerId,$planId,$targetExpiry);
    ...
    public static function guardcore_refresh_catalog(): void {
        ...
        $r=BlueVPN_تأمین‌کنندهs::guardcore_catalog($id,true);
    ...
    public static function guardcore_bootstrap_key(): void {
        ...
        $r=BlueVPN_تأمین‌کنندهs::guardcore_bootstrap_api_key($id,$code);
    ...
    public static function guardcore_node_action(): void {
        ...
        $r=BlueVPN_تأمین‌کنندهs::guardcore_node_action($panelId,$nodeId,$enable);
        self::redirect('guardcore',$r['message'],empty($r['ok']));
    }
    ...
    public static function guardcore_subscription_action(): void {
        ...
        $r=BlueVPN_تأمین‌کنندهs::guardcore_subscription_action($panelId,$username,$op);
        if(!empty($r['ok'])&&$customerId>0)BlueVPN_تأمین‌کنندهs::request_background_sync($customerId);
        self::redirect('guardcore',$r['message'],empty($r['ok']));
    }
    ...
    public static function refresh_guardcore_stats(): void {
        ...
        $queued=0;
        foreach($ids as $id){
            if(BlueVPN_تأمین‌کنندهs::request_background_snapshot((int)$id))$queued++;
        }
        self::redirect('guardcore','بروزرسانی آمار GuardCore برای '.$queued.' کاربر در صف قرار گرفت.');
    }

    public static function attach_guardcore(): void { self::guard();$id=(int)($_POST['customer_id']??0);check_admin_referer('bluevpn_cc_attach_guardcore_'.$id);$r=BlueVPN_تأمین‌کنندهs::attach_guardcore($id,(string)wp_unslash($_POST['subscription_url']??''));self::redirect('guardcore-manual',$r['message'],!$r['ok']); }
