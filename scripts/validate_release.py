...
def main() -> None:
    ...
    require(version == "4.1.3" and code == 40103, "this hotfix source must be 4.1.3 / 40103")
    ...
    require("Version: 4.1.3" in manager, "WordPress plugin header version mismatch")
    require("BLUEVPN_MANAGER_VERSION', '4.1.3'" in manager, "WordPress plugin constant mismatch")
    require("BLUEVPN_MANAGER_SCHEMA_VERSION', '1.6.0'" in manager, "WordPress schema must be 1.6.0")
    require("Version: 4.1.3" in manager_readme and "Stable tag: 4.1.3" in manager_readme, "WordPress readme version/stable tag mismatch")
    ...
    require("handler.postDelayed(attemptTimeout, 12_000L)" in home, "Xray start window is not 12 seconds")
    require("round < 3" in home, "tunnel verification must have three rounds")
    require("maxWaitMs: Long = 2_400L" in home, "local proxy readiness window is not hardened")
