...
def main() -> None:
    ...
    require(version == "4.1.6" and code == 40106, "this compatibility-hardening source must be 4.1.6 / 40106")
    ...
    require("Version: 4.1.6" in manager, "WordPress plugin header version mismatch")
    require("BLUEVPN_MANAGER_VERSION', '4.1.6'" in manager, "WordPress plugin constant mismatch")
    require("BLUEVPN_MANAGER_SCHEMA_VERSION', '1.6.0'" in manager, "WordPress schema must be 1.6.0")
    require("Version: 4.1.6" in manager_readme and "Stable tag: 4.1.6" in manager_readme, "WordPress readme version/stable tag mismatch")
    ...
    require("round < 3" in home, "tunnel verification must have three rounds")
    ...
    require("SettingsManager.getSocksPort()" in home, "tunnel verifier does not use canonical SOCKS port")
    require("AppConfig.PREF_DYNAMIC_SOCKS_PORT" in home and "false" in function_block(home, "    private fun enforceReliableVpnSettings()", "    private fun renderVerifyingState()"), "cross-process dynamic SOCKS port is not disabled")
    ...
    require("startVServiceExact" in engine, "exact-candidate v2rayNG bridge missing")
    ...
    require("result.status && result.guid == guid && result.content.isNotBlank()" in engine, "runtime config hydration does not prove exact GUID/content")
    require("startCoreLoop(vpnInterface: ParcelFileDescriptor?, requestedGuid: String? = null)" in prepare, "daemon core start does not accept exact GUID")
    require("doStartCoreLoop(service, vpnInterface, requestedGuid)" in prepare, "exact GUID is not forwarded into doStartCoreLoop")
    require("CoreServiceManager.startCoreLoop(mInterface, requestedGuid)" in prepare, "CoreVpnService does not pass its exact target directly to CoreServiceManager")
    require("blueVpnTargetGuid = requestedGuid" in prepare, "CoreVpnService does not retain the intent target for direct daemon handoff")
    ...
    require("!terminalFailureStopping" in function_block(home, "        mainViewModel.updateTestResultAction.observe(this) { result ->", "    private fun parseV2rayNgDelayMs"), "late v2rayNG ping can mark a terminally failed route connected")
