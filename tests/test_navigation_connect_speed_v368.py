...
def test_release_368():
    ...
    assert r['version']=='3.0.68'
    assert r['version_code']==30068
    assert r['android_version']=='3.0.68'
    assert r['android_version_code']==30068
    assert a['version_name']=='3.0.68'
    assert a['version_code']==30068
...
def test_navigation_is_immediate_and_auto_unlocks():
    ...
    assert 'handler.postDelayed(navigationUnlock, 1_200L)' in home
...
def test_fast_free_connection_timer_and_probe():
    home=(ROOT/'android-source/BlueVpnHomeActivity.kt').read_text()
    assert 'beginFreeTimerOnCoreStart()' in home
    assert 'SystemClock.elapsedRealtime() + 1_350L' in home
    assert 'connection.connectTimeout = 850' in home
    assert 'connection.readTimeout = 850' in home
    assert 'handler.postDelayed(attemptTimeout, 3_200L)' in home
